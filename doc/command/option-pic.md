...
The difference between `-fpic` and `-fPIC` is that `-fPIC` generates **GOTX** relocations for global data accesses. In contrast, `-fpic` generates **GOT** relocations. Additionally, the option `-fno-plt` can also be used in conjunction with `-fpic` or `-fPIC` to avoid using the Procedure Linkage Table (PLT) for function calls. Option `-fplt` enables the use of the PLT for function calls.
