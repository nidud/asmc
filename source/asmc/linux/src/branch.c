...
int process_branch( struct code_info *CodeInfo, unsigned CurrOpnd, const struct expr *opndx )
/************************************************************************************************/
/*
 ...
 */
{
	...
	if ( SegOverride && opndx->sym && opndx->sym->segment ) {
		...
		return( asmerr( 2074, opndx->sym ? opndx->sym->name : "" ) );
	    }
	...
	if(  ( CodeInfo->mem_type == MT_EMPTY ||
	   ...
	   CodeInfo->isfar == FALSE ) {
		...
		if( CodeInfo->opnd[OPND1].type != OP_I8 ) {
			...
			return( asmerr( CodeInfo->mem_type == MT_EMPTY ? 2075 : 2079, addr ) );
		    }
		}
	    }
