 ...
 */
int RunMacro( struct dsym *macro, int idx, struct asm_tok tokenarray[],
	char *out, int mflags, bool *is_exitm )
{
    char	*currparm;
    ...
    *is_exitm = FALSE;
	...
	if ( tokenarray[idx].token == T_FINAL ||
	    ...
	    *currparm = NULLC;
		...
		if ( tokenarray[idx].token == T_PERCENT ) {
		    ...
		    while ( islspace( *(tokenarray[idx].tokpos+cnt-1) ) ) cnt--;
		...
		if ( !inside_literal && ( tokenarray[idx+1].token == T_COMMA ||
		    ...
		    while ( islspace( *(tokenarray[idx].tokpos+i-1 ) ) ) i--;
		}
	...
	if ( tokenarray[idx].token != T_CL_BRACKET ) {
		...
		asmerr( 4006, macro->sym.name, tokenarray[i].tokpos );
	    }
	}
	idx++;
	...
	if ( !(mflags & MF_IGNARGS) )
	    asmerr( 4006, macro->sym.name, tokenarray[idx].tokpos );
	//return( -1 );
    }
	...
	*is_exitm = TRUE;
	...
	while ( GetTextLine( CurrSource ) ) {
		...
		if ( tokenarray[1].token != T_ID )
		    asmerr(2008, tokenarray[0].tokpos );
		else if ( tokenarray[2].token != T_FINAL )
		    asmerr(2008, tokenarray[2].tokpos );
		continue;
	    }
		...
		if ( tokenarray[0].tokval == T_EXITM || tokenarray[0].tokval == T_RETM ) {
		    ...
		    *is_exitm = TRUE;
			...
			if ( !lnode ) {
			    ...
			    asmerr( 2147, tokenarray[1].string_ptr );
			} else {
...
int ExpandText( char *line, struct asm_tok tokenarray[], unsigned int substitute )
/*************************************************************************************/
{
    char *pSrc;
    char *pDst;
...
*/
static int RebuildLine( const char *newstring, int i, struct asm_tok tokenarray[], unsigned oldlen, unsigned pos_line, int addbrackets )
/*******************************************************************************************************************************************/
{
    char *dest;
    const char *src;
 ...
 */
int ExpandToken( char *line, int *pi, struct asm_tok tokenarray[], int max,
	int bracket_flags, int equmode, char *buffer )
{
    int pos;
    int tmp;
    int i = *pi;
	...
	if( tokenarray[i].token == T_ID ) {
		...
		if ( sym->state == SYM_MACRO ) {
			 ...
			 */
			if ( i == 0 || ( i == 2 && ( tokenarray[1].token == T_COLON ||
			     tokenarray[1].token == T_DBL_COLON )) || ( i == 1 && sym->label ))
			    ;
			else {
			    continue;
			}
...
int ExpandLineItems( char *line, int i, struct asm_tok tokenarray[], int addbrackets, int equmode )
{
	...
	for( k = i; k < Token_Count; ) {
	    tmp = ExpandToken( line, &k, tokenarray, Token_Count, addbrackets, equmode, buffer );
	    if ( tmp == ERROR )
		return( lvl );
 ...
 */
int ExpandLine( char *string, struct asm_tok tokenarray[] )
{
    int count;
	...
	if ( tokenarray[i].token == T_DIRECTIVE ) {
		...
		if ( tokenarray[i].dirtype == DRT_ERRDIR ) {
			...
			if ( i )
			    rc = ExpandToken( string, &count, tokenarray, 1, FALSE, FALSE, buffer );
		...
		rc = ExpandToken( string, &count, tokenarray, 2, FALSE, FALSE, buffer );
		if( rc == ERROR || rc == EMPTY )
		    return( rc );
		if ( rc == STRING_EXPANDED ) {
		    Token_Count = Tokenize( string, 0, tokenarray, TOK_DEFAULT );
	...
	while ( count < Token_Count ) {
	    ...
	    tmp = ExpandToken( string, &count, tokenarray, Token_Count, addbrackets, FALSE, buffer );
