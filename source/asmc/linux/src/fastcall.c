	...
	bool addr, struct expr *opnd, char *paramvalue, uint_8 *r0used )
{
    ...
    size = 4;
    if ( opnd->mem_type != MT_EMPTY )
	size = SizeFromMemtype( opnd->mem_type, USE_EMPTY, opnd->type );
	...
	if ( reg[i] ) {
		...
		if ( a == 0 || b == 0 ) {
		    ...
		    if ( ModuleInfo.Ofssize )
			AddLineQueueX( "mov %r, %s", reg_32, paramvalue );
		    else
		...
		t = psize;
		if ( t < 4 && ModuleInfo.Ofssize ) {
		    a = reg_32;
		    t = 4;
		}
		if ( a != b ) {
		    if ( ( SizeFromRegister( b ) < 4 || t < 4 ) && ModuleInfo.Ofssize ) {
			qual = T_MOVSX;
			if ( !( param->sym.mem_type & MT_SIGNED ) )
			    qual = T_MOVZX;
			AddLineQueueX( "%r %r, %r", qual, a, b );
		    } else
			AddLineQueueX( "mov %r, %r", a, b );
		}
	    } else {
		if ( i == 0 && reg[1] == NULL ) {
		    if ( ( size < psize || psize < 4 ) && ModuleInfo.Ofssize ) {
			t = size;
			if ( size > psize )
			    t = psize;
			opc = T_BYTE;
			...
			if ( !( param->sym.mem_type & MT_SIGNED ) )
			    qual = T_MOVZX;
			 AddLineQueueX( " %r %r, %r ptr %s", qual, reg_32, opc, paramvalue );
		    } else {
			AddLineQueueX( "mov %s, %s", reg[i], paramvalue );
		    }
		} else {
		    qual = ( ModuleInfo.Ofssize ) ? T_DWORD : T_WORD;
