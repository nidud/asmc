...
struct asym {
    ...
    unsigned char   vtable:1,	  /* base class - first member = lpVtbl */
		    isvtable:1,	  /* vtable struct - classVtbl */
		    vmacro:1,	  /* vtable method is inline - sym->vmacro set */
		    isinline:1,	  /* v2.31.24: SYM_EXTERNAL.target_type == SYM_MACRO */
		    isstatic:1,	  /* v2.32.26: SYM_EXTERNAL.target_type == SYM_MACRO */
		    vparray:1;	  /* p.p.proc() -- nested function call */
...
struct seg_info {
    ...
    unsigned		segtype;	/* segment's type (code, data, ...) */
    int			lname_idx;	/* segment's name LNAME index (OMF only) */
    struct asym		*clsym;		/* segment's class name (stored in an asym item) */
    union {
	uint_32		abs_frame;	/* ABS seg, frame number (OMF,BIN) */
	uint_32		comdat_number;	/* associated COMDAT segno (COFF) */
	uint_32		comdat_idx;	/* lname index of COMDAT symbol (OMF) */
    };
    union {
    ...
    unsigned char	readonly;	/* if segment is readonly */
    unsigned char	info;		/* if segment is info only (COFF/ELF) */
    unsigned char	force32;	/* force 32bit segdef (OMF only) */
    unsigned char	data_in_code;	/* data items in code segm (OMF only) */
    unsigned char	internal;	/* internal segment with private buffer */
    unsigned char	written;	/* code/data just written */
    unsigned char	linnum_init;	/* v2.10: linnum data emitted for segment? */
    unsigned char	combine;	/* combine type, see omfspec.h */
    unsigned char	comdat_selection; /* if > 0, it's a COMDAT (COFF/OMF) */
};
