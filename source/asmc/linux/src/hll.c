...
static struct asym *GetProcVtbl( struct asym *sym, char *name )
{
    uint_32 poffset;
    ...
    if ( sym->target_type &&
        ...
        sym = sym->target_type;
    if ( sym->vtable )
        sym = SearchNameInStruct( sym->segment, name, &poffset, 0 );
    return ( sym );
}

static struct asym *GetProc( int i, struct asm_tok tokenarray[], struct expr *opnd )
{
    struct asym *sym, *target;
    int j;

    if ( tokenarray[i].token == T_OP_SQ_BRACKET ) {
        for ( j = i; tokenarray[i].token != T_FINAL; i++ ) {
            if ( tokenarray[i].token == T_CL_SQ_BRACKET )
                break;
        }
        i += 2;
        if ( tokenarray[i+1].token != T_DOT )
            return NULL;
        if ( EvalOperand( &j, tokenarray, i + 1, opnd, 0 ) == ERROR )
            return NULL;
        sym = opnd->type;
    } else if ( tokenarray[i].token == T_REG ) {
        sym = GetStdAssume( GetRegNo( tokenarray[i].tokval ) );
    } else {
    ...
    if ( tokenarray[i].token   == T_ID &&
        ...
        return GetProcVtbl( sym, tokenarray[i+2].string_ptr );
...
static struct dsym *GetParamId( int i, struct dsym *sym )
{
    struct proc_info *info = sym->e.procinfo;
    int lang = sym->sym.langtype;
    struct dsym *curr = info->paralist;
    int k = ( lang == LANG_STDCALL || lang == LANG_C || lang == LANG_SYSCALL ||
    ...
    while ( curr && i ) {
        ...
        if ( k ) {
            struct dsym *d = curr;
            for ( curr = info->paralist; curr && curr->nextparam != d;
                curr = curr->nextparam )
                ;
        } else {
            curr = curr->nextparam;
...
static char *GetMacroReturn( int i, struct asm_tok tokenarray[] )
{
  char mac_name[128];
  struct expr opnd;
  struct asym *sym;
  ...
  struct srcline *line;
    ...
    if ( sym->state == SYM_STRUCT_FIELD ) {
        ...
        if ( tokenarray[i].token == T_OP_SQ_BRACKET )
            ...
            t = SymFind( tokenarray[i].string_ptr );
            if ( t == NULL )
                return NULL;
            if ( t->mem_type == MT_TYPE )
                t = t->type;
            else if ( t->mem_type == MT_PTR )
                t = t->target_type;
            else
                t = NULL;
        }
        if ( t == NULL || t->typekind != TYPE_STRUCT )
            return NULL;

        strcat( strcat( strcpy( mac_name, t->name ), "_" ), sym->name );
        if ( ( t = SymFind( mac_name ) ) == NULL )
            return NULL;
        if ( t->state == SYM_TMACRO ) {
            if ( ( t = SymSearch( t->string_ptr ) ) == NULL )
                return NULL;
        }

        sym->vmacro = 1;
        sym->segment = t; /* vtable method is inline - sym->vmacro set */
    ...
    if ( t->state != SYM_MACRO )
        return NULL;

    line = ((struct dsym *)t)->e.macroinfo->data;
    if ( line == NULL )
        return NULL;
...
static int StripSource( int i, int e, struct asm_tok tokenarray[] )
{
    ...
    for ( p = b, j = 0; j < i; j++ ) {
        ...
        k = ( tokenarray[j+1].tokpos - tokenarray[j].tokpos );
    ...
    mac = GetMacroReturn( i, tokenarray );
    if ( mac != NULL )
        goto macro_args;

    if ( proc_id /*&& tokenarray[proc_id].token != T_OP_SQ_BRACKET*/ ) {

        if ( ( sym = (struct dsym *)GetProc( proc_id, tokenarray, &opnd ) ) ) {

            curr = GetParamId( parg_id, sym );
...
static int LKRenderHllProc( char *dst, int i, struct asm_tok tokenarray[] )
{
    ...
    for ( j = i; tokenarray[j].token != T_FINAL; j++ ) {
        if ( tokenarray[j].token == T_OP_SQ_BRACKET ) {
            for ( sqcount++, j++; sqcount && tokenarray[j].token != T_FINAL; j++ ) {
                if ( tokenarray[j].hll_flags & T_HLL_PROC ) {
                    ...
                    brcount--;
                } else if ( !brcount && tokenarray[j].token == T_OP_SQ_BRACKET ) {
                    sqcount++;
                } else if ( !brcount && tokenarray[j].token == T_CL_SQ_BRACKET ) {
                    sqcount--;
                    if ( sqcount == 0 ) {
                        if ( !sqbrend )
                            sqbrend = j;
        ...
        } else if ( tokenarray[j].token == T_DOT ) {
            dots[dotcount++] = j;
        } else if ( tokenarray[j].token == T_OP_BRACKET )
            break;
    }
    if ( tokenarray[j].token != T_OP_BRACKET )
        return asmerr( 2008, tokenarray[j].string_ptr );
    ...
    if ( tokenarray[i].token == T_OP_SQ_BRACKET ) {
        ...
        if ( dotcount ) {
            ...
            if ( dotcount > 1 ) {
                ...
                target = SymFind( tokenarray[k+1].string_ptr );

            } else if ( tokenarray[k+2].token == T_OP_BRACKET ) {

                /* [reg].Method() -- assume reg:ptr Class */

                name = tokenarray[k+1].string_ptr;
                brcount = i;
                if ( EvalOperand( &brcount, tokenarray, dots[0], &opnd, 0 ) != ERROR )
                    target = opnd.type;
            }
            if ( target ) {
        ...
        for ( x = 0; sym && k; x++, k-- ) {
            tmp = sym;
            if ( tmp->mem_type == MT_TYPE )
                tmp = tmp->type;
            if ( tmp->mem_type == MT_PTR )
                tmp = tmp->target_type;
            if ( tmp == NULL )
                break;
            i = dots[x];
        ...
        if ( vparray ) {
            if ( sym1 != sym && sym1->mem_type != MT_PTR ) {
                if ( dotcount == 2 )
                    vparray = NULL;  /* static class.ptr.proc() */
                else
                    static_struct++; /* static class.ptr.ptr...proc() */
            }
        }
        ...
        if ( tmp ) {
            ...
            if ( sym->mem_type == MT_TYPE && sym->type ) {
                ...
                if ( target->typekind == TYPE_TYPEDEF ) {
                    ...
                    tmp = NULL;
                    if ( sym->vtable )
                        tmp = SearchNameInStruct( sym->segment, name, &u, 0 );
                    if ( tmp == NULL ) {
                        tmp = SearchNameInStruct( sym, name, &u, 0 );
                        if ( tmp ) {
                            sym = NULL;
                            method = tmp;
                        }
    ...
    if ( target && target->state == SYM_TYPE && target->typekind == TYPE_STRUCT ) {

        comptr = target->name;
        tmp = SearchNameInStruct( target, name, &u, 0 );
        if ( tmp )
            method = tmp;
        if ( target->vtable ) {
            tmp = SearchNameInStruct( target->segment, name, &u, 0 );
            if ( tmp ) {
                vtable++;
                method = tmp;
    ...
    if ( comptr ) {
        ...
        if ( ModuleInfo.Ofssize == USE64 && !constructor ) {
            ...
            tmp = method;
            if ( tmp ) { /* v2.28: Added for undefined symbol error.. */
        ...
        if ( tokenarray[i].token == T_OP_SQ_BRACKET ) {
            ...
            if ( tokenarray[i+2].token == T_CL_SQ_BRACKET ) {
                ...
                for ( k = i + 1; k < sqbrend; k++ )
                    strcat( comptr, tokenarray[k].string_ptr );
            }

        } else if ( dotcount ) {

            comptr = ClassVtbl;
            k = ( tokenarray[args-2].tokpos - tokenarray[i].tokpos );
            memcpy( comptr, tokenarray[i].tokpos, k );
            comptr[k] = '\0';
        } else
    ...
    p = b + strlen( b );
    x = ( tokenarray[args].tokpos - tokenarray[i].tokpos );
    memcpy( p, tokenarray[i].tokpos, x );
    p[x] = '\0';
    i = args + 1;

    if ( type ) {
        strcat( b, ", 0" );
    } else if ( comptr && vtable ) {
        strcat( b, ", " );
        if ( static_struct )
            strcat( b, "addr " );
        p = b + strlen( b );
    ...
    for ( brcount = 1; tokenarray[i].token != T_FINAL; ) {
        ...
        if ( tokenarray[i].token == '&' ) {
            ...
            x = 5;
                ...
                tokenarray[i+1].token == T_CL_BRACKET && brcount == 1 ) {
            j++;
            i++;
            break;
        } else {
            p = tokenarray[i].tokpos;
            x = tokenarray[i+1].tokpos - p;
        }
        q = b + strlen( b );
        memcpy( q, p, x );
        q[x] = '\0';
    ...
    if ( *dst )
        strcat( dst, EOLSTR );
    strcat( dst, b );
    return StripSource( id, j, tokenarray );
}
