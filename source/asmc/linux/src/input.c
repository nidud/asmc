...
int InputExtend( struct line_status *p )
{
    ...
    if ( p->start != srclinebuffer ) {
	 ...
	 q = p->start;
	 p->start = LclAlloc( ModuleInfo.g.max_line_len );
	 if ( p->start == NULL )
	     return 0;

	memcpy( p->start, q, len );
	p->input = p->input - q + p->start;
    }
    if ( AllocInput() == 0 )
	return 0;
    ...
    if ( p->output == stringbuf )
	p->output = p->output - stringbuf + token_stringbuf;
    if ( p->start == linebuffer ) {
	p->input = p->input - p->start + srclinebuffer;
	p->start = srclinebuffer;
    }
    if ( tokenarray == p->tokenarray ) {

	int t = srclinebuffer - linebuffer;
	int s = token_stringbuf - stringbuf;

	q = stringbuf + len;
	p->tokenarray = ModuleInfo.tokenarray;
	tokenarray = ModuleInfo.tokenarray;
	for ( i = 0; i <= p->index; i++ ) {
	    tokenarray[i].tokpos += t;
	    if ( tokenarray[i].string_ptr >= stringbuf && tokenarray[i].string_ptr <= q )
		tokenarray[i].string_ptr += s;
	}
    }
    return 1;
}
