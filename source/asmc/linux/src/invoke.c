...
/* [rbx*8+[rdi].foo(edx)].bar(...) */

static int SkipSQBackets( int i, struct asm_tok tokenarray[] )
{
   int brackets = 0;
   int sqbrackets = 1; /* first token */

    for ( i++; sqbrackets && tokenarray[i].token != T_FINAL; i++ ) {
	if ( tokenarray[i].token == T_OP_BRACKET )
	    brackets++;
	else if ( tokenarray[i].token == T_CL_BRACKET )
	    brackets--;
	else if ( !sqbrackets && tokenarray[i].token == T_OP_SQ_BRACKET )
	    sqbrackets++;
	else if ( !sqbrackets && tokenarray[i].token == T_CL_SQ_BRACKET ) {
	    sqbrackets--;
	    if ( sqbrackets == 0 )
		return( i + 1 );
	}
...
static int GetSQBackets( int i, struct asm_tok tokenarray[], char *buffer )
{
    int x;
    int e = SkipSQBackets( i, tokenarray );

    if ( e == 0 )
	return 0;

    x = ( tokenarray[e].tokpos - tokenarray[i].tokpos );
    memcpy( buffer, tokenarray[i].tokpos, x );
    buffer[x] = '\0';
    return e;
}

struct asym *FindDotSymbol( int i, struct asm_tok tokenarray[] )
{
    int j = i;
    struct asym *sym;
    uint_32 u;

    while ( tokenarray[i-1].token != T_COMMA && tokenarray[i-1].token != T_DIRECTIVE )
	i--;
    if ( ( sym = SymSearch( tokenarray[i].string_ptr ) ) == NULL )
	return NULL;
	...
	if ( tokenarray[i].token == T_OP_SQ_BRACKET ) {
	    if ( ( i = SkipSQBackets( i, tokenarray ) ) == 0 )
		break;
	}
	...
	sym = SearchNameInStruct( sym, tokenarray[i].string_ptr, &u, 0 );
	if ( sym == NULL )
	    return NULL;
	if ( i == j )
	    return sym;
...
static void AssignPointer( struct asym *sym, int reg, int i, struct asm_tok tokenarray[],
	struct asym *pclass, int langtype )
{
  char buffer[128];
  int vreg;
  int vtable;
  ...
  uint_32 u;
	...
	if ( tokenarray[i].token == T_REG ) {
		...
		return;
	    AddLineQueueX( " mov %r, %r", reg, tokenarray[i].tokval );
	} else
	...
	if ( r ) {
	    ...
	    i++;
	    if ( tokenarray[i].token == T_OP_SQ_BRACKET ) {
		if ( ( i = GetSQBackets( i, tokenarray, buffer ) ) == 0 )
		    return;
	    ...
	    if ( sym->mem_type == MT_PTR )
		sym = sym->target_type;
	    if ( sym == NULL || sym->state != SYM_TYPE )
		return;

	    AddLineQueueX( " mov %r, [%r].%s.%s", reg, reg, sym->name, tokenarray[i].string_ptr );
	    sym = SearchNameInStruct( sym, tokenarray[i].string_ptr, &u, 0 );
	    if ( sym == NULL )
		return;
	    vtable = 1;
    ...
    if ( !vtable )
	AddLineQueueX( " mov %r, %s", reg, sym->name );
...
int InvokeDirective( int i, struct asm_tok tokenarray[] )
{
	...
	if ( sym->state == SYM_EXTERNAL &&
		...
		*(struct asym **)macro->altname = macro;
		...
		pclass = SymSearch( tokenarray[namepos+4].string_ptr );
		if ( pclass && pclass->isvtable ) {
		    if ( opnd.mbr->vmacro ) {
			macro = opnd.mbr->segment;
			strcpy( buffer, macro->name );
		    } else {
			strcat( strcat( strcpy( buffer, pclass->segment->name ), "_" ), opnd.mbr->name );
	...
	*/
	...
	if ( macro ) {
	    ...
	    AssignPointer( SymSearch( tokenarray[struct_ptr].string_ptr ),
		reg, struct_ptr, tokenarray, pclass, sym->langtype );
		...
		reg = T_EAX; /* v2.31 - skip mov eax,reg */
		if ( sym->langtype == LANG_WATCALL )
		    ; /* do nothing: class in eax */
		else if ( sym->langtype == LANG_FASTCALL )
		    /* class in ecx */
		    reg = T_ECX;
		else if ( tokenarray[parmpos+1].tokval != T_EAX ) {
