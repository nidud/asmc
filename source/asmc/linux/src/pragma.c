...
int PragmaDirective( int i, struct asm_tok tokenarray[] )
{
    ...
    if ( _stricmp(p, "asmc" ) == 0 ) {
        ...
        if ( _stricmp( tokenarray[i].string_ptr, "lib" ) == 0 ) {
        ...
        } else if ( _stricmp( tokenarray[i].string_ptr, "linker" ) == 0 ) {
            ...
            if ( *q == '"' ) {
                ...
                if ( Parse_Pass == PASS_1 ) { /* do all work in pass 1 */
                    ...
                    item = (struct qitem *)LclAlloc( sizeof( struct qitem ) + ( p - s ) );
                    strcpy( item->value, dynlib );
                    QEnqueue( &ModuleInfo.g.LibQueue, item );
                } else {
                    while ( tokenarray[i].token == T_STRING )
                        i++;
                }
            }
        }
