...
static int get_string( struct asm_tok *buf, struct line_status *p )
{
    ...
    case '"':
    case '\'':
	maxlen = ModuleInfo.g.max_line_len - 32;
		...
		if ( p->flags == TOK_DEFAULT && (( p->flags2 & DF_NOCONCAT ) == 0 ) ) { /* <{ */
			...
			if ( p->index > 1 &&
			     ( tokenarray[0].tokval == T_DOT_STATIC ||
			       ...
			       tokenarray[0].tokval == T_DOT_INLINE ||
...
static int get_special_symbol( struct asm_tok *buf, struct line_status *p )
{
	...
	if ( p->index == 2 && (buf-2)->tokval == T_DOT_PRAGMA ) {

	    p->cstring++;

	} else if ( *(p->input+1) == ')' && (buf-1)->token == T_REG ) {
...
int GetToken( struct asm_tok token[], struct line_status *p )
{
    int x, b, i;
	...
	unsigned int flags )
{
    ...
    p.flags2 = 0;
    p.flags3 = 0;
    p.outbuf = token_stringbuf;
    p.tokenarray = tokenarray;
    p.cstring = 0;
    p.brachets = 0;
	...
	if( *p.input == NULLC ) {
	     ...
	     */
	    if ( p.index > 1 &&
		( tokenarray[p.index-1].token == T_COMMA || p.brachets )
    ...
    if ( p.brachets && flags != TOK_RESCAN && CurrIfState == BLOCK_ACTIVE )
	asmerr( 2157 );
