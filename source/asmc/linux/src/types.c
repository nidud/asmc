...
/************************************************************/
{
	...
	char *p = ModuleInfo.g.ComStack->class;
	int len = strlen( p );
	if ( memcmp( p, name, len ) == 0 ) {
	    if ( name[len] == 0 ) {
		ModuleInfo.g.ComStack->sym = (struct asym *)dir;
	    } else if ( strcmp( &name[len], "Vtbl" ) == 0 ) {
		ModuleInfo.g.ComStack->sym->vtable = 1;
		ModuleInfo.g.ComStack->sym->segment = (struct asym *)dir;
		dir->sym.segment = ModuleInfo.g.ComStack->sym;
		dir->sym.isvtable = 1;
	    }
	}
    }
