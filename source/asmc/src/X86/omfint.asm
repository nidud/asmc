...
func_index db \
    ...
    assume esi:ptr omf_rec
    assume ebx:ptr outbuff
...
PutWord proc watcall private value:word
    ...
    mov ecx,[ebx].in_buf
    add [ebx].in_buf,2
    mov word ptr [ebx].buffer[ecx],ax
    ret
PutWord endp
...
PutDword proc watcall private value:dword
    ...
    mov ecx,[ebx].in_buf
    add [ebx].in_buf,4
    mov dword ptr [ebx].buffer[ecx],eax
    ret
PutDword endp
...
PutBase proc watcall private uses edi base:ptr base_info
    ;
    ; write public base field of COMDAT, PUBDEF or LINNUM records
    ;
    mov edi,base
    PutIndex( [edi].base_info.grp_idx )
    PutIndex( [edi].base_info.seg_idx )
    .if ( [edi].base_info.grp_idx == 0 && [edi].base_info.seg_idx == 0 )
        PutWord( [edi].base_info.frame )
    .endif
    ret
PutBase endp
...
omf_write_record proc uses esi edi ebx objr:ptr omf_rec
  ...
  local o:outbuff
    ...
    lea     ebx,o
    mov     esi,objr
    movzx   ecx,[esi].command
    sub     ecx,CMD_MIN_CMD
    shr     ecx,1
    movzx   eax,func_index[ecx]
    ...
    .case OFF_SEGDEF
       ...
       .new alig:byte
        ...
        ; P=32bit
        ;
        mov al,[esi].d.segdef.combine
        shl al,2
        or  al,[esi].d.segdef.use_32
        mov cl,[esi].d.segdef._align
        mov alig,cl
        shl cl,5
        or  al,cl
        ;
        ...
        ;
        .if ( is32 == 0 && [esi].d.segdef.seg_length == 0x10000 )
            or al,0x02
        .endif
        ;
        ...
        .if ( alig == SEGDEF_ALIGN_ABS )
            ...
            ;
            PutWord( [esi].d.segdef.abs.frame )
            PutByte( [esi].d.segdef.abs.offs )
        .endif
        .if ( is32 )
            PutDword( [esi].d.segdef.seg_length )
        .else
            PutWord( [esi].d.segdef.seg_length )
        .endif
        PutIndex( [esi].d.segdef.seg_lname_idx )
        PutIndex( [esi].d.segdef.class_lname_idx )
        PutIndex( [esi].d.segdef.ovl_lname_idx )
        jmp write_end
    .case OFF_LEDATA
        ;
        ...
        add al,[esi].is_32
        mov o.cmd,al
        PutIndex( [esi].d.ledata.idx )
        .if ( [esi].is_32 )
            PutDword( [esi].d.ledata.offs )
        .else
            PutWord( [esi].d.ledata.offs )
        .endif
        .endc
    .case OFF_COMENT
        mov o.cmd,CMD_COMENT
        PutByte( [esi].d.coment.attr )
        PutByte( [esi].d.coment.cmt_class )
        .endc
    .case OFF_THEADR
        .gotosw(OFF_MISC)
    .case OFF_MODEND
        xor eax,eax
        .if ( [esi].is_32 && [esi].d.modend.start_addrs )
            inc eax
        .endif
        add eax,CMD_MODEND
        mov o.cmd,al
        ...
        ;
        xor eax,eax
        .if ( [esi].d.modend.main_module )
            mov eax,0x80
        .endif
        .if ( [esi].d.modend.start_addrs )
            or eax,0x41
            PutByte( eax )
            .endc
        .endif
    ...
    .case OFF_PUBDEF
        ...
        add al,[esi].is_32
        mov o.cmd,al
        PutBase( &[esi].d.pubdef.base )
        .endc
    .case OFF_LINNUM
        mov al,CMD_LINNUM
        add al,[esi].is_32
        mov o.cmd,al
        PutBase( &[esi].d.linnum.base )
        .endc
    .case OFF_COMDAT
        ;
        ...
        add al,[esi].is_32
        mov o.cmd,al
        PutByte( [esi].d.comdat.flags )
        PutByte( [esi].d.comdat.attributes )
        PutByte( [esi].d.comdat._align )
        .if ( [esi].is_32 )
            PutDword( [esi].d.comdat.offs )
        .else
            PutWord( [esi].d.comdat.offs )
        .endif
        PutIndex( [esi].d.comdat.type_idx )
        mov al,[esi].d.comdat.attributes
        and eax,COMDAT_ALLOC_MASK
        .if ( eax == COMDAT_EXPLICIT )
            PutBase( &[esi].d.comdat.base )
        .endif
        PutIndex( [esi].d.comdat.public_lname_idx )
        ;
    ...
    .case OFF_LINSYM
        ...
        add al,[esi].is_32
        mov o.cmd,al
        PutByte( [esi].d.linsym.flags )
        PutIndex( [esi].d.linsym.public_lname_idx )
        .endc
    .endsw
    ;
...
write_end:
    ...
    .endf
    neg al
    mov [edx],al ;; store chksum in buffer
    ;
    ; write buffer + 4 extra bytes (1 cmd, 2 length, 1 chksum)
    ;
    mov ebx,o.in_buf
    add ebx,4
    .if ( fwrite( &o.cmd, 1, ebx, CurrFile[OBJ*4] ) != ebx )
        WriteError()
    .endif
    ret
