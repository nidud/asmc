...
OrgDirective proc i:int_t, tokenarray:ptr asm_tok
  ...
  local opndx:expr
    ...
    .endif
    .return .if ( EvalOperand( &i, tokenarray, Token_Count, &opndx, cl ) == ERROR )

    imul ecx,i,asm_tok
    add ecx,tokenarray
    .if ( [ecx].asm_tok.token != T_FINAL )
        .return( asmerr( 2008, [ecx].asm_tok.string_ptr ) )
    .endif
    .if ( CurrStruct )
    ...
    .else
        ...
        .if ( Parse_Pass == PASS_1 && [ecx].seg_info.head )
            mov edx,[ecx].seg_info.head
            or [edx].fixup.fx_flag,FX_ORGOCCURED
        .endif
        ...
        .elseif ( opndx.kind == EXPR_ADDR && !( opndx.flags & E_INDIRECT ) )
            ...
            add ecx,opndx.value
            .return( SetCurrOffset( CurrSeg, ecx, FALSE, FALSE ) )
        .endif
    .endif
    .return( asmerr( 2132 ) )
OrgDirective endp

fill_in_objfile_space proc private uses esi edi ebx size:dword
    ...
    .if ( [esi].seg_info.written == FALSE )

        SetCurrOffset( edi, size, TRUE, TRUE )

    .elseif ( [esi].seg_info.segtype != SEGTYPE_CODE )
    ...
    .else
        ...
        .endw
        .return .if ( size == 0 )
        inc esi ;; here i is the index into the NOP table
...
AlignDirective proc i:int_t, tokenarray:ptr asm_tok
  ...
  local CurrAddr:uint_t
    ...
    .case T_ALIGN
        ...
        .elseif ( opndx.kind == EXPR_EMPTY )  ;; ALIGN without argument?
            ...
            .if ( CurrStruct )
                ...
                mov align_value,[ecx].struct_info.alignment
            .else
    ...
    .if ( Parse_Pass == PASS_1 && CurrSeg )
        ...
        mov ecx,CurrSeg
        mov ecx,[ecx].dsym.seginfo
        mov ecx,[ecx].seg_info.head
        .if ecx
            or  [ecx].fixup.fx_flag,FX_ORGOCCURED
        .endif
    .endif
