...
static unsigned __fastcall GetLinnumItems( struct qdesc *q )
/***********************************************/
{
    unsigned i;
...
static ret_code __fastcall coff_write_section_table( struct module_info *modinfo, struct coffmod *cm )
/*****************************************************************************************/
{
    struct dsym	 *curr;
    struct fixup *fix;
 ...
 */
static short __fastcall CoffGetType( const struct asym *sym )
/************************************************/
{
    if ( sym->isproc )
	return( 0x20 );
 ...
 */
static short __fastcall CoffGetClass( const struct asym *sym )
/*************************************************/
{
...
static unsigned __fastcall GetFileAuxEntries( uint_16 file, char * *fname )
/**************************************************************/
{
 ...
 */
static uint_32 __fastcall coff_write_symbols( struct module_info *modinfo, struct coffmod *cm )
/**********************************************************************************/
{
...
static int __fastcall GetStartLabel( char *buffer, bool msg )
/************************************************/
{
    int size = 0;
...
/**************************************************************************************/
{
	...
	if ( currsize ) {
	    p = (unsigned char *)LclAlloc( currsize + sizeof( struct qditem ) );
 ...
 */
static uint_32 __fastcall SetSymbolIndices( struct module_info *ModuleInfo, struct coffmod *cm )
/***********************************************************************************/
{
    struct qnode *q;
    struct dsym	 *curr;
...
/**************************************************************************************/
{
	...
	if ( section->e.seginfo->Ofssize == USE64 ) {
		...
		break;
	    default:
		asmerr( 3014, fix->type, section->sym.name, fix->locofs );
...
static ret_code __fastcall coff_write_data( struct module_info *modinfo, struct coffmod *cm )
/********************************************************************************/
{
    struct dsym *section;
...
static void __fastcall coff_create_drectve( struct module_info *modinfo, struct coffmod *cm )
/********************************************************************************/
{
    struct dsym *exp;
...
static int coff_write_module( struct module_info *modinfo )
/**************************************************************/
{
	...
	if ( cm.sxdata = (struct dsym *)CreateIntSegment( ".sxdata", "", MAX_SEGALIGNMENT, modinfo->Ofssize, FALSE ) ) {
	    ...
	    cm.sxdata->e.seginfo->CodeBuffer = (unsigned char *)LclAlloc( cnt*4 );
	}
    }
