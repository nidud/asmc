...
static cv_typeref __fastcall GetTyperef( struct asym *sym, uint_8 Ofssize )
/**************************************************************/
{
...
static uint_16 __fastcall GetCVStructLen( struct asym *sym, uint_8 Ofssize )
/***************************************************************/
{
    uint_16 len;
    switch ( sym->state ) {
...
static void __fastcall PadBytes( uint_8 *curr, uint_8 *base )
/************************************************/
{
...
static cv_typeref __fastcall cv_write_ptr_type( struct dbgcv *cv, struct asym *sym )
/************************************************************************/
{
...
static uint_16 __fastcall cv_get_register( struct asym *sym )
/************************************************/
{
    uint_16 regno;
    uint_16 rc = 0;
...
static uint_16 __fastcall cv_get_x64_regno( uint_16 regno )
/**********************************************/
{
...
static void __fastcall cv_write_symbol( struct dbgcv *cv, struct asym *sym )
/***************************************************************/
{
    int	       len;
    unsigned   ofs;
	...
	for ( i = 0; i < 2 ; i++ ) {
		...
		if ( lcl->sym.state == SYM_TMACRO ) {
		    ...
		    if (
			Ofssize == USE64 || ( GetRegNo( proc->e.procinfo->basereg ) != 5 )) {
