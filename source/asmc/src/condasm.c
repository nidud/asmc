...
/************************************************/
{
	...
	if ( blocknestlevel ) { /* v2.04: do nothing if there was no IFx */
	    ...
	    asmerr( 1007 ); //, GetResWName( directive, NULL ) );
	}
	break;
    case T_ENDIF:
	if ( blocknestlevel ) {
...
/*************************************************************/
{
	...
	if ( tokenarray[i].token != T_FINAL ) {
	    asmerr( 8005, tokenarray[i-1].tokpos );
...
/***********************************************************/
{
	...
	if ( errmsg != EMPTY )
	    asmerr( errmsg, opndx.value, GetErrText( erridx, tokenarray ) );
	break;
	...
	if ( direct == T_DOT_ERRB && check_blank( string1 ) )
	    ...
	    errmsg = 2058;
	if ( errmsg != EMPTY )
	    asmerr( errmsg, string1, GetErrText( erridx, tokenarray ) );
	break;
	...
	if ( errmsg != EMPTY )
	    asmerr( errmsg, string1, string2, GetErrText( erridx, tokenarray ) );
	break;
    case CC_PASS2: /* .ERR2 */
	if ( ModuleInfo.setif2 == FALSE ) {
	    return( asmerr( 2061 ) );
	...
	if ( Parse_Pass == PASS_1 )
	    break;
	asmerr( 2052, GetErrText( erridx, tokenarray ) );
	break;
    }
    if ( tokenarray[i].token != T_FINAL ) {
	return( asmerr(2008, tokenarray[i].tokpos ) );
