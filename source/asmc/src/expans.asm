...
ExpandClass proc __ccall uses rsi rdi rbx buffer:string_t, string:string_t, tokenarray:token_t

    ldr rbx,tokenarray

    mov rax,CurrSeg
    mov rcx,CurrProc
    .if ( rcx && [rcx].asym.ClassProc && TokenCount > 1 && rax == [rcx].asym.segm &&
        !( ProcStatus & ( PRST_INSIDE_PROLOGUE or PRST_INSIDE_EPILOGUE ) ) )

        .for ( rdi = [rcx].asym.class : [rbx].token != T_FINAL : rbx+=asm_tok )
    ...
    .for ( lvl = 0: lvl < MAX_TEXTMACRO_NESTING: lvl++ )
        ...
        .if ( TokenCount > 2 )
            ...
            .if ( [rbx+asm_tok].token == T_COLON || [rbx+asm_tok].token == T_DBL_COLON )
                ...
                .if ( [rbx].token != T_ID || [rbx+asm_tok].token == T_COLON || [rbx+asm_tok*2].token != T_ID )
        ...
        .if ( [rbx].token == T_DIRECTIVE )
            ...
            .if ( TokenCount == 1 && [rbx].tokval == T_ENDP )

                mov rcx,CurrProc
                .if ( rcx )
                    tsprintf( string, "%s %r", [rcx].asym.name, T_ENDP )
                    mov TokenCount,Tokenize( string, 0, tokenarray, TOK_DEFAULT )
                .endif
                .break
            .endif
        ...
        .if ( TokenCount > 2 && [rbx].Expand )
            ...
            .if ( [rbx].IsProc )
                ...
                .if ( [rbx].token == T_ID )
                    ...
                    .endif
                .endif
                tsprintf( buffer, "%r %s", esi, [rbx].tokpos )
            ...
            .for ( : [rbx].token != T_FINAL : rbx += asm_tok )
                ...
                .if ( [rbx+asm_tok].token == T_DBL_COLON )
                    ...
                    .if ( [rbx+3*asm_tok].token == T_DIRECTIVE && ( ecx == T_PROC || ecx == T_PROTO ) )
        ...
        .endif
        .if ( MODULE.class_reg )
            ExpandClass( buffer, string, tokenarray )
        .endif
