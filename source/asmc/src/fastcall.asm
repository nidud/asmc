...
ms32_param proc uses esi edi ebx pp:ptr dsym, index:SINT, param:ptr dsym, adr:SINT,
	...
	.else
		...
		.else
		    ...
		    AddLineQueueX(" mov %r, %s", addr [edi+T_AL], paramvalue)
		    AddLineQueueX(" mov %r, 0",	 addr [edi+T_AH])
		.endif
	    .else
...
;;
	...
	xor eax,eax
	.if reg[1*4] != eax
	    .if GetSegmentPart(opnd, addr buffer, paramvalue)
		AddLineQueueX("%r %s, %r", T_MOV, reg, eax)
	    .else
		AddLineQueueX("%r %s, %s", T_MOV, reg, addr buffer)
	    .endif
	    mov eax,4
	.endif
	mov eax,reg[eax]
...
ms64_param proc uses esi edi ebx pp:ptr dsym, index:SINT, param:ptr dsym, adr:SINT,
	...
	check_register_overwrite(
		edi, regs_used, addr reg, addr destroyed, REGPAR_WIN64 )
	...
	.if esi >= 4
		...
		or [ecx],al
		AddLineQueueX(" lea %r, %s", ebx, paramvalue)
		AddLineQueueX(" mov [%r+%u], %r", T_RSP, addr [esi*8], ebx)
		mov eax,1
		.break
	    .endif
		...
		.ifs psize == 8 && (cl || ch)
			...
			T_DWORD, T_RSP, addr [esi*8], T_LOW32, paramvalue)
		    AddLineQueueX(" mov %r ptr [%r+%u], %r ( %s )",
			T_DWORD, T_RSP, addr [esi*8+4], T_HIGH32, paramvalue)
		.else
		    ;
			...
			ecx, T_RSP, addr [esi*8], paramvalue)
		.endif
		...
		.if [edx].asym.mem_type == MT_REAL8
			...
			T_DWORD, T_RSP, addr [esi*8], T_HIGH32, paramvalue)
		.else
		    AddLineQueueX(" mov %r ptr [%r+%u], %s",
			T_DWORD, T_RSP, addr [esi*8], paramvalue)
		.endif
		...
		.elseif [edi].expr.kind != EXPR_REG || [edi].expr.flags & EXF_INDIRECT
		    AddLineQueueX(" mov %r, %s", i, paramvalue)
		.endif
		AddLineQueueX(" mov [%r+%u], %r", T_RSP, addr [esi*8], i)
	    .endif
	...
	.elseif [edx].asym.mem_type == MT_REAL4 || \
		...
		.if [edx].asym.mem_type == MT_REAL4
		    ...
		    AddLineQueueX(" movd %r, %r", addr [esi+T_XMM0], T_EAX)
		.else
		    AddLineQueueX(" mov %r, %r ptr %s", T_RAX, T_REAL8, paramvalue)
		    AddLineQueueX(" movd %r, %r", addr [esi+T_XMM0], T_RAX)
		.endif
	    .else
		.if [edx].asym.mem_type == MT_REAL4
		    AddLineQueueX(" movd %r, %s", addr [esi+T_XMM0], paramvalue)
		.else
		    AddLineQueueX(" movq %r, %s", addr [esi+T_XMM0], paramvalue)
		.endif
	    .endif
