...
static ret_code omf_write_autodep( void )
{
	...
	if ( len > 255 )
	    len = 255; /* length is 1 byte only */
#endif
	*((time_t *)p) = timet2dostime( GetFileTimeStamp( q ) );
...
static int omf_write_header_initial( struct module_info *modinfo )
/*********************************************************************/
{
    uint_16 ext_idx;
