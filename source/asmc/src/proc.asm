...
fastcall_conv       struct
    paramcheck      proc :ptr dsym, :ptr dsym, :ptr int_t
    handlereturn    proc :ptr dsym, :string_t
fastcall_conv       ends
...
watc_regname        sbyte 64 dup(0)
watc_regist         sbyte 32 dup(0)
...
vc32_pcheck proc private uses esi edi ebx p:ptr dsym, paranode:ptr dsym, used:ptr int_t
...
ms64_pcheck proc private p:ptr dsym, paranode:ptr dsym, used:ptr int_t
...
ms64_return proc private p:ptr dsym, buffer:string_t
...
WriteSEHData proc private uses esi edi ebx p:ptr dsym
   ...
   .new buffer[128]:char_t
    ...
    .if ( [esi].exc_handler )
        mov ecx,[esi].exc_handler
        AddLineQueueX( "dd IMAGEREL %s", [ecx].asym.name )
    ...
    AddLineQueueX( "dd IMAGEREL %s, IMAGEREL %s+0%xh, IMAGEREL $xdatasym+0%xh",
                  [edi].asym.name, [edi].asym.name, [edi].asym.total_size, xdataofs )
    AddLineQueueX("%s ENDS", segname )
...
if STACKPROBE
        .if ( Options.chkstack && [esi].localsize > 0x1000 )
            AddLineQueue( "externdef _chkstk:PROC" )
            AddLineQueueX( "mov eax, %d", [esi].localsize )
            AddLineQueue( "call _chkstk" )
...
runqueue:
    ...
    .if ( ModuleInfo.Ofssize == USE64 && ModuleInfo.fctype == FCT_ELF64 && \
        ...
        AddLineQueueX( " mov DWORD PTR [rsp+8], %d", &[ebx*8] )
