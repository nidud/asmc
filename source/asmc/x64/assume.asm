...
OperandSize proto __ccall :int_t, :ptr code_info
...
SetSegAssumeTable proc __ccall uses rsi rdi savedstate:ptr
...
GetSegAssumeTable proc __ccall uses rsi rdi savedstate:ptr
...
SetStdAssumeTable proc __ccall uses rsi rdi savedstate:ptr, ti:ptr stdassume_typeinfo

    UNREFERENCED_PARAMETER(savedstate)
...
AssumeInit proc __ccall pass:int_t ;; pass may be -1 here!

    UNREFERENCED_PARAMETER(pass)

    .for ( rdx = &SegAssumeTable,
           eax = 0,
           r8d = 0 : r8d < NUM_SEGREGS : r8d++, rdx += assume_info )
        mov [rdx].symbol,rax
        mov [rdx].error,al
        mov [rdx].is_flat,al
    .endf
    ...
    .if ( ecx <= PASS_1 ) ; v2.10: just reset assumes in pass one

        .for ( rdx = &StdAssumeTable,
               r8d = 0 : r8d < NUM_STDREGS : r8d++, rdx += assume_info )
...
ModelAssumeInit proc __ccall
    ...
    .case MODEL_FLAT
        ...
        .if ( ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_VEC64 )
            lea r8,szNothing
        .endif
        AddLineQueueX( "assume cs:flat,ds:flat,ss:flat,es:flat,fs:%s,gs:%s", rdx, r8 )
        .endc
...
GetStdAssume proc __ccall reg:int_t
...
GetStdAssumeEx proc __ccall reg:int_t
...
AssumeDirective proc __ccall uses rsi rdi rbx r12 i:int_t, tokenarray:token_t
    ...
    .for ( : i < Token_Count : i++, rbx += asm_tok )
        ...
        .elseif ( segtable == FALSE )
            ...
            .if ( rax == NULL )
                ...
                mov [r12+rsi*8],CreateTypeSymbol(NULL, "", FALSE)
...
search_assume proc __ccall uses rsi rdi rbx sym:asym_t, def:int_t, search_grps:int_t
    ...
    mov rdi,rcx
    mov ebx,edx
    mov rsi,GetGroup(rcx)
    lea r8,SegAssumeTable
    ...
    .for ( r9 = &searchtab, ecx = 0 : ecx < NUM_SEGREGS : ecx++ )

        mov eax,[r9+rcx*4]
        imul edx,eax,assume_info

       .return .if ( rdi == [r8+rdx].symbol )
    .endf
    ...
    .if ( search_grps && rsi )
        ...
        .for ( ecx = 0 : ecx < NUM_SEGREGS : ecx++ )
            ...
            .return .if ( [r8+rdx].is_flat && rsi == ModuleInfo.flat_grp )
            .return .if ( rsi == [r9+rdx].symbol )
        .endf
    .endif
    .return( ASSUME_NOTHING )
...
GetOverrideAssume proc __ccall override:int_t
...
GetAssume proc __ccall override:asym_t, sym:asym_t, def:int_t, passume:ptr asym_t
    ...
    .if ( rcx != NULL )
        ...
        search_assume( rcx, eax, FALSE )

        ; v2.10: added

    .elseif ( [r10].asym.state == SYM_STACK )
        ...
        mov eax,ASSUME_SS
    .else
        search_assume( [r10].asym.segm, eax, TRUE )
    .endif
