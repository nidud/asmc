...
GetSegRelocs proc __ccall uses rsi rdi rbx r12 pDst:ptr uint_16
   ...
   .new count:int_t = 0
    ...
    .for ( rsi = SymTables[TAB_SEG*symbol_queue].head : rsi : rsi = [rsi].dsym.next )
        ...
        mov rdi,[rsi].dsym.seginfo
        .continue .if ( [rdi].segtype == SEGTYPE_ABS )
...
GetImageSize proc __ccall uses rsi rdi rbx memimage:int_t
    ...
    .for ( rsi = SymTables[TAB_SEG*symbol_queue].head,
        ...
        mov rdi,[rsi].dsym.seginfo
        .continue .if ( [rdi].segtype == SEGTYPE_ABS || [rdi].info )
        .if ( memimage == FALSE )
...
DoFixup proc __ccall uses rsi rdi rbx r12 r13 r14 r15 curr:ptr dsym, _cp:ptr calc_param
  ...
  local offs:uint_32 ; v2.07
    ...
    .return( NOT_ERROR ) .if ( [rdi].segtype == SEGTYPE_ABS )

    .for ( rbx = [rdi].head : rbx : rbx = [rbx].nextrlc )

        mov r15d,[rbx].locofs
        sub r15d,[rdi].start_loc
        add r15,[rdi].CodeBuffer
        ...
        .if ( rcx && ( [rcx].asym.segm || [rcx].asym.flags & S_VARIABLE ) )
            ...
            .case FIX_OFF32_IMGREL
                ...
                sub eax,[r14].calc_param.imagestart
                mov value,eax
                .endc
            ...
            .case FIX_OFF32_SECREL
                ...
                .if ( tstrchr( [r13].asym.name, '$' ) )
                    ...
                    .for ( r12 = SymTables[TAB_SEG*symbol_queue].head : r12 : r12 = [r12].dsym.next )
                        ...
                        .if ( [r12].asym.name_size == edx )

                            mov     r8,rsi
                            mov     r9,rdi
                            mov     rdi,[r12].asym.name
                            mov     rsi,[r13].asym.name
                            mov     ecx,edx
                            xor     eax,eax
                            repz    cmpsb
                            setnz   al
                ...
                .if ( [rsi].sgroup && [rbx].frame_type != FRAME_SEG )
                    ...
                    .if ( ModuleInfo.sub_format == SFORMAT_PE || ModuleInfo.sub_format == SFORMAT_64BIT )
                        ...
                        .if ( [rdi].Ofssize == USE64 )
                            ...
                            mov eax,value
                            add rax,[r14].calc_param.imagebase64
                            mov value64,rax
                        .endif
                        add value,[r14].calc_param.imagebase
                    .endif
                .else
                    mov eax,[rsi].start_offset
                    and eax,0xF
        ...
        .case FIX_PTR16
            ...
            .if ( r13 && [rsi].segtype == SEGTYPE_ABS )
                mov [r15],ax
    ...
    .if ( Parse_Pass == PASS_1 )
        ...
        .for ( ebx = 1, esi = 0: ebx < SIZE_PEFLAT: ebx++ )
            ...
            .for ( rdi = SymTables[TAB_SEG*symbol_queue].head : rdi : rdi = [rdi].dsym.next )
                ...
                .continue .if ( [rdx].seg_info.segtype != [rcx+rbx*4] )
...
pe_emit_export_data proc __ccall uses rsi rdi rbx
  ...
  local pitems:ptr expitem
    ...
    ;
    .for ( rbx = CurrFName[OBJ*8], rbx += tstrlen( rbx ): rbx > CurrFName[OBJ*8]: rbx-- )
...
bin_write_module proc __ccall uses rsi rdi rbx modinfo:ptr module_info
    ...
    .return( ERROR ) .if ( [rbx].error_count )
    ...
    .if ( cp.sizehdr )
        ...
        fwrite( hdrbuf, 1, cp.sizehdr, CurrFile[OBJ*8] )
        .if ( eax  != cp.sizehdr )
            WriteError()
        .endif
