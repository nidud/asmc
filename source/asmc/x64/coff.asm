...
GetLinnumItems proc __ccall q:ptr qdesc

    UNREFERENCED_PARAMETER(q)

    .for ( eax = 0,
           rdx = [rcx].qdesc.head: rdx: eax++, rdx = [rdx].line_num_info.next )
    .endf
    ret
...
CoffGetType proc __ccall sym:ptr asym
...
CoffGetClass proc __ccall sym:ptr asym
...
CRC32Comdat proc __ccall lpBuffer:ptr byte, dwBufLen:uint_32, dwCRC:uint_32
...
coff_write_symbol proc __ccall name:string_t, strpos:dword, value:dword,
  ...
  local sym:IMAGE_SYMBOL
    ...
    .ifd ( fwrite( &sym, 1, sizeof( IMAGE_SYMBOL ), CurrFile[OBJ*8] ) != sizeof( IMAGE_SYMBOL ) )
        WriteError()
    .endif
    ret
...
if COMPID
    ...
    .for ( rdi = ModuleInfo.PubQueue.head: rdi: rdi = [rdi].qnode.next )
        ...
        .if ( Options.line_numbers && Options.debug_symbols != 4 && \
            [rsi].asym.flag1 & S_ISPROC && [rcx].debug_info.file != lastfile )
            ...
            .for ( rdx = p, eax = aux: eax: eax--, rdx += sizeof(IMAGE_AUX_SYMBOL) )
            .endf
...
coff_write_fixup proc __ccall adr:uint_32, index:uint_32, type:uint_16
  ...
  local ir:IMAGE_RELOCATION
    ...
    mov ir.SymbolTableIndex,edx
    mov ir.Type,r8w

    .ifd ( fwrite( &ir, 1, sizeof(ir), CurrFile[OBJ*8] ) != sizeof(ir) )
        WriteError()
    .endif
    ret
...
coff_write_data proc __ccall uses rsi rdi rbx modinfo:ptr module_info, cm:ptr coffmod
  ...
  .new index:uint_32
    ...
    mov index,SetSymbolIndices( rcx, rdx )
...
coff_create_drectve proc __ccall uses rsi rdi rbx modinfo:ptr module_info, cm:ptr coffmod
  ...
  .new buffer[MAX_ID_LEN + MANGLE_BYTES + 1]:char_t
    ...
    .if ( Options.write_impdef && !Options.names[OPTN_LNKDEF_FN*8] )
...
coff_write_module proc __ccall uses rsi rdi rbx modinfo:ptr module_info
  ...
  local ifh:IMAGE_FILE_HEADER
    ...
    mov cm.dot_file_value,CurrFName[ASM*8]
    ...
    .if ( Options.debug_symbols )
        ...
        .for ( ebx = 0: ebx < DBGS_MAX: ebx++ )
            ...
            mov cm.SymDeb[rdi].s,CreateIntSegment( [rcx+rbx*8], "", 0, USE32, TRUE )
           .break .if ( eax == NULL )
...
ifdef __UNIX__
    ...
    .ifd ( fwrite( &ifh, 1, sizeof( ifh ), CurrFile[OBJ*8] ) != sizeof( ifh ) )
        WriteError()
    .endif
    .return( NOT_ERROR )
...
coff_init proc __ccall public modinfo:ptr module_info
