...
GetTyperef proc __ccall uses rsi sym:ptr asym, Ofssize:byte
    ...
    .if ( !( cl & MT_SPECIAL ) )
        SizeFromMemtype( cl, dl, [rsi].asym.type )
        mov cl,[rsi].asym.mem_type
        .if ( cl & MT_FLOAT )
            .switch eax
        ...
        .else
            ...
            .else
                ...
                .case 16 : .return ST_UINT8
                .case 2
                    ;.if ( [rsi].asym.flag1 & S_ISARRAY && Options.debug_symbols == CV_SIGNATURE_C13 )
    ...
    .else
        ...
        .case MT_TYPE
            ...
            .if ( [rsi].asym.cvtyperef )
                .return( [rsi].asym.cvtyperef )
            .endif
            .return( GetTyperef( rsi, Ofssize ) )
        .endsw
    .endif
    .return( ST_NOTYPE )
GetTyperef endp

SetPrefixName proc __ccall p:ptr byte, name:ptr byte, len:dword
...
GetStructLen proc __ccall sym:ptr asym, Ofssize:byte
...
dbgcv::flushpt proc __ccall uses rbx size:dword
...
dbgcv::flushps proc __ccall uses rbx size:dword
...
dbgcv::padbytes proc __ccall curr:ptr byte

    UNREFERENCED_PARAMETER(this)
    UNREFERENCED_PARAMETER(curr)
...
dbgcv::write_array_type proc __ccall uses rsi rdi rbx r12 sym:ptr asym, elemtype:dword, Ofssize:byte
    ...
    .if ( r8d == 0 )
        mov elemtype,GetTyperef( r12, r9b )
    .endif

    xor esi,esi
    .if ( [r12].asym.total_size >= LF_NUMERIC )
    ...
    .else
        ...
        mov [rdi].CV_ARRAY.idxtype,GetTyperef( r12, Ofssize )
...
dbgcv::write_ptr_type proc __ccall uses rsi rdi rbx r12 sym:ptr asym
    ...
    .else ; pointer to simple type
        mov r12b,[rsi].asym.mem_type      ; the target type is tmp. copied to mem_type
        mov al,[rsi].asym.ptr_memtype   ; thus GetTyperef() can be used
        mov [rsi].asym.mem_type,al
        GetTyperef( rsi, [rsi].asym.Ofssize )
        mov [rsi].asym.mem_type,r12b
    .endif
    .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
...
dbgcv::memberproc proc __ccall uses rsi rdi rbx r12 r13 r14 type:ptr dsym, mbr:ptr asym, cc:ptr counters
    ...
    .if ( [rcx].asym.typekind != TYPE_RECORD )
        add r13d,[r14].asym.offs
        add r13d,[rsi].counters.ofs
    .endif
    .if ( r13d >= LF_NUMERIC )
        add r12d,sizeof( uint_32 )
    .endif
    ...
    .endif
    add edx,r12d
    mov ecx,[r14].asym.name_size
    lea rsi,[rdx+rcx+2+1+3]
    and esi,not 3
    mov rdi,[rbx].flushpt(esi)
    add [rbx].pt,rsi
    mov eax,LF_MEMBER
    ...
    .if ( [r14].asym.flag1 & S_ISARRAY )
        movzx eax,[r14].asym.ext_idx1
        mov [r14].asym.ext_idx1,0 ; reset the temporarily used field
    .else
        GetTyperef( r14, USE16 )
    .endif
    .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
    ...
    .else
        ...
        add rcx,6
    .endif
    SetPrefixName( rcx, [r14].asym.name, [r14].asym.name_size )
    [rbx].padbytes(rax)
    ret
...
dbgcv::enum_fields proc __ccall uses rsi rdi rbx r12 r13 symb:ptr dsym, enumfunc:cv_enum_func, cc:ptr counters

    UNREFERENCED_PARAMETER(symb)
    UNREFERENCED_PARAMETER(cc)
    ...
    .for ( rdi = [rcx].struct_info.head, ebx = 0: rdi: rdi = [rdi].sfield.next )
        .if ( [rdi].sfield.name_size ) ; has member a name?
            enumfunc( this, rsi, rdi, r12 )
        .elseif ( [rdi].sfield.type )  ; is member a type (struct, union, record)?
            add [r12].counters.ofs,[rdi].sfield.offs
            this.enum_fields( [rdi].sfield.type, enumfunc, r12 )
            sub [r12].counters.ofs,[rdi].sfield.offs
...
dbgcv::write_type_procedure proc __ccall uses rsi rdi rbx r12 r13 sym:ptr asym, cnt:int_t
    ...
    .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
        ...
        mov [rdi].CV_PROCEDURE_16t.parmcount,r13w
    ...
    .else
        ...
        mov [rdi].CV_PROCEDURE.parmcount,r13w
...
getregister proc __ccall uses rsi rdi rbx sym:ptr asym
...
get_x64_regno proc __ccall regno:dword
...
dbgcv::write_symbol proc __ccall uses rsi rdi rbx r12 r13 r14 sym:ptr asym
...
if EQUATESYMS
    ...
    .if ( [rsi].asym.flag1 & S_ISPROC && Options.debug_ext >= CVEX_REDUCED ) ;; v2.10: no locals for -Zi0
        ...
        .for ( r12d = 0: r12d < 2: r12d++ )

            .if ( ModuleInfo.defOfssize == USE64 && r12d == 0 )
                .for ( r13d = 0, rcx = locals[0*dsym_t]: rcx: rsi = rcx, rcx = [rcx].dsym.nextparam )
                    inc r13d
                .endf
            .endif

            mov cnt[r12*int_t],0
            .for ( rdi = locals[r12*dsym_t]: rdi: rdi = [rdi].dsym.nextparam )
                ...
                .if ( ModuleInfo.defOfssize == USE64 && r12d == 0 )
                    .fors ( ecx = 1, r13d--, rsi = locals[0]: r13d > ecx: ecx++, rsi = [rsi].dsym.nextparam )
                    .endf
                .endif
            .endf
        .endf
    ...
    .if ( [rsi].asym.flag1 & S_ISPROC && Options.debug_ext >= CVEX_REDUCED )
        ...
        .for ( r12d = 0: r12d < 2 : r12d++ )
            .if ( ModuleInfo.defOfssize == USE64 && r12d == 0 )
                .for ( r13d = 0, rcx = locals[0]: rcx: rsi = rcx, rcx = [rcx].dsym.nextparam )
                    inc r13d
                .endf
            .endif
            .for ( rdi = locals[r12*dsym_t]: rdi: rdi = [rdi].dsym.nextparam )
                ...
                .if ( [rsi].asym.state == SYM_TMACRO )
                    mov r14d,sizeof( REGSYM ) - 1
                    mov leaf,S_REGISTER
                    .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
                        mov r14d,sizeof( REGSYM_16t ) - 1
                    ...
                    mov eax,[rsi].asym.name_size
                    lea eax,[rax+r14-sizeof(uint_16)+1]
                ...
                .else
                    ...
                    .if ( Ofssize == USE64 || ( GetRegNo( ecx ) != 5 ))
                        ...
                        mov r14d,sizeof( REGREL32 ) - 1
                        mov leaf,S_REGREL32
                        .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
                            mov r14d,sizeof( REGREL32_16t ) - 1
                        ...
                        lea r8,SpecialTable
                        .if ( [r8+rax].special_item.cpu == P_64 )
                        ...
                        .endif
                    .else
                        mov r14d,sizeof( BPRELSYM32 ) - 1
                        mov leaf,S_BPREL32
                        .if ( Options.debug_symbols == CV_SIGNATURE_C7 )
                            mov r14d,sizeof( BPRELSYM32_16t ) - 1
                ...
                .if ( ModuleInfo.defOfssize == USE64 && r12d == 0 )
                    .for ( ecx = 1, r13d--, rsi = locals[0]: r13d > ecx: ecx++, rsi = [rsi].dsym.nextparam )
                    .endf
                .endif
            .endf
        .endf
...
dbgcv::flush_section proc __ccall uses rsi rdi rbx signature:dword, ex:dword
    ...
    sub rsi,rax
    lea rcx,[rsi + r8 + sizeof( qditem ) + sizeof( cvsection )]
...
F proto fastcall x:dword, y:dword, z:abs { ; (((x) & (y)) | ((~x) & (z)))
    xor y,z
    and x,y
    xor x,z
    }

G proto fastcall x:dword, y:dword, z:abs { ; (((x) & (z)) | ((y) & (~z)))
    xor x,y
    and x,z
    xor x,y
    }

H proto fastcall x:dword, y:dword, z:abs { ; ((x) ^ (y) ^ (z))
    xor x,y
    xor x,z
    }

I proto fastcall x:dword, y:dword, z:abs { ; ((y) ^ ((x) | (~z)))
    not z
    or  x,z
    not z
    xor x,y
...
MD5Transform proc __ccall buf:ptr dword, inp:ptr byte
...
MD5Update proc __ccall uses rsi rdi rbx ctx:ptr MD5_CTX, buf:ptr byte, len:dword
...
MD5Final proc __ccall uses rsi rdi rbx ctx:ptr MD5_CTX
...
calc_md5 proc __ccall uses rsi rdi rbx filename:string_t, sum:ptr byte
  ...
  local ctx:MD5_CTX
    ...
    .while 1
        .break .if !fread( rbx, 1, MD5BUFSIZ, rsi )
        MD5Update( rdi, rbx, eax )
    .endw
