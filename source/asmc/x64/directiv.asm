...
StubDir proc fastcall i:int_t, tokenarray:token_t
...
IncludeLibDirective proc __ccall uses rbx i:int_t, tokenarray:token_t
    ...
    .if ( [rbx].token == T_STRING && [rbx].string_delim == '<' )
       ...
       .return asmerr(2008, [rbx+asm_tok].tokpos) .if ( [rbx+asm_tok].token != T_FINAL )
...
IncBinDirective proc __ccall uses rsi rsi rbx i:int_t, tokenarray:token_t
  ...
  local opndx:expr
    ...
    .return asmerr(2008, [rbx].tokpos) .if ( [rbx].token != T_FINAL )
