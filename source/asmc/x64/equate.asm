...
check_number:
    ...
    .else
        ...
        .if ( opnd.kind != EXPR_CONST && \
            ...
            .elseif ( !rcx && opnd.kind == EXPR_FLOAT && !ModuleInfo.strict_masm_compat )
...
CreateConstant proc uses rsi rdi rbx tokenarray:token_t
    ...
    .case [rbx].tokval == T_DEFINE && [rsi].token == T_FINAL
        ...
        ;
        xor eax,eax
        mov opnd.llvalue,rax
        mov opnd.hlvalue,rax
        mov opnd.sym,rax
        dec i
    ...
    .if ( rc != ERROR && [rbx+rax].token == T_FINAL && ( ( opnd.kind == EXPR_CONST \
        && rdx == 0 ) || ( opnd.kind == EXPR_ADDR && !( opnd.flags & E_INDIRECT ) \
        && rcx != NULL && [rcx].state == SYM_INTERNAL ) ) && ( opnd.inst == EMPTY ) )
