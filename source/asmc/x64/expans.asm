...
RunMacro proc __ccall uses rsi rdi rbx r12 r13 r14 mac:dsym_t, idx:int_t, tokenarray:token_t,
    ...
    .if ( [rsi].mac_flag & M_LABEL )
        .if ( mflags & MF_LABEL )
            ...
            rep movsb
            mov r13,GetAlignedPointer( r13, i )
        .else
            mov ecx,parmidx
            mov rdx,mi.parm_array
            lea rax,@CStr("")
        ...
        .else
            ...
            .for ( r12 = rdx : : idx++ )
                ...
                .if ( [rbx].token == T_PERCENT )
                    ...
                    .ifd ExpandText( r12, tokenarray, FALSE ) == ERROR
                    ...
                    .if cnt_opnum
                        ...
                        mov edx,Token_Count
                        inc edx
                        mov max,Tokenize( r12, edx, tokenarray, TOK_RESCAN )
                        mov i,Token_Count
                        inc i
                    ...
                    .if ( [rbx].token == T_ID )
                        .if SymSearch( [rbx].string_ptr )
                            ...
                            .if ( [rax].asym.state == SYM_MACRO && [rax].asym.flags & S_ISDEFINED &&
                                ...
                                inc idx
                                mov idx,RunMacro( sym, idx, tokenarray, r12, 0, &is_exitm2 )
                                .if idx < 0
            ...
            .elseif byte ptr [rdx]
                ...
                mov rcx,r12
                sub rcx,rdx
                mov r13,GetAlignedPointer( r13, ecx )
            .else
                mov rdx,mi.parm_array
                mov ecx,parmidx
                lea rax,@CStr("")
                ...
                .elseif ( [rbx].tokval == T_GOTO )
                    ...
                    .if ( [rbx+asm_tok].token != T_FINAL )
                        ...
                        .for ( i = 1, rdi = mi.startline: rdi != NULL: rdi = [rdi].next, i++ )
                            ...
                            .if ( byte ptr [rsi] == ':' )
                                .if ( [rdi].ph_count )
                                    fill_placeholders(StringBufferEnd, rsi, mi.parmcnt, mi.localstart, mi.parm_array)
                                    mov rsi,StringBufferEnd
                                .endif
                                inc rsi
                                .while islspace([rsi])
...
if sizeof(asm_tok) ne 24
.err @CatStr(%@FileName,<(>,%@Line,<): >, <asm_tok need to 24 byte!!!!>)
endif
...
AddTokens proc __ccall private uses rbx tokenarray:token_t, start:int_t, count:int_t, _end:int_t
    ...
    .ifs r8d > 0
        ...
        add  rbx,rax
        .for( : r9d >= edx: r9d--, rbx -= asm_tok )
            mov [rbx+rcx],qword ptr [rbx]
            mov [rbx+rcx].string_ptr,[rbx].string_ptr
            mov [rbx+rcx].tokpos,[rbx].tokpos
        .endf
    .elseifs r8d < 0
        sub  edx,r8d
        imul eax,edx,asm_tok
        add  rbx,rax
        .for( : edx <= r9d: edx++, rbx += asm_tok )
            mov [rbx+rcx],qword ptr [rbx]
            mov [rbx+rcx].string_ptr,[rbx].string_ptr
            mov [rbx+rcx].tokpos,[rbx].tokpos
        .endf
...
ExpandTMacro proc __ccall private uses rsi rdi rbx outbuf:string_t, tokenarray:token_t, equmode:int_t, level:int_t
...
if TEVALUE_UNSIGNED
        ...
        mov ecx,opndx.value
        myltoa( rcx, StringBufferEnd, ModuleInfo.radix, FALSE, FALSE )
else
        xor eax,eax
        .if opndx.hvalue < 0
            mov eax,1
