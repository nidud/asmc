...
RenderInstr proc fastcall uses rsi rdi rbx dst:string_t, inst:string_t, start1:uint_t,
...
GetLabelStr proc fastcall public l_id:int_t, buffer:string_t

    tsprintf( buffer, "@C%04X", ecx )
   .return buffer
...
;
RenderJcc proc uses rdi dst:string_t, cc:int_t, _neg:int_t, _label:int_t
    ;
...
;
LGetToken proc private uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok, opnd:ptr expr
    ;
    ...
    .for ( : edi < ModuleInfo.token_count,
           GetCOp(rbx) == COP_NONE : edi++, rbx += asm_tok )
    .endf
...
GetSimpleExpression proc private uses rsi rdi rbx \
        hll:        ptr hll_item,
        i:          ptr int_t,
   ...
   .new op2_end:int_t = edi
    ...
    .elseif ( ecx <= COP_LE )
        ...
        .if ( !rax && ( ecx == COP_EQ || ecx == COP_NE ) && op1.kind == EXPR_REG && \
        ...
        .if ( edi || edx == MT_SIGNED || eax == MT_SIGNED )
            lea r8,signed_cjmptype
            movzx edx,byte ptr [r8+rcx-COP_EQ]
        .else
            lea r8,unsign_cjmptype
            movzx edx,byte ptr [r8+rcx-COP_EQ]
        .endif

        mov eax,is_true
        lea r8,neg_cjmptype
        .if ( !byte ptr [r8+rcx-COP_EQ] )
            xor eax,1
        .endif
...
;
ReplaceLabel proc private uses rsi rdi rbx p:ptr, olabel, nlabel
...
GetAndExpression proc private uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok,
  ...
  local truelabel:int_t, nlabel:int_t, olabel:int_t, buff[16]:char_t
    ...
    .while 1
        ...
        .if rbx && is_true
            ...
            .if B[rbx]
                ...
                strcat(GetLabelStr(truelabel, &[rbx+4]), &EOLSTR)
            .endif
            ...
            tstrlen(rbx)
            add rbx,rax
            mov r8,GetLabelStr(olabel, &buff)
            tsprintf(rbx, "%s%s%s", r8, LABELQUAL, &EOLSTR)
    ...
    .if truelabel
        tstrlen(rsi)
        add rsi,rax
        strcat(strcat(GetLabelStr(truelabel, rsi), LABELQUAL), &EOLSTR)
        mov [rdi].hll_opnd.lastjmp,0
    .endif
    mov eax,NOT_ERROR
    ret
...
GetExpression proc private uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok,
    ...
    .while 1
        ...
        .if ( rbx && !is_true )
            ...
            .if ( [rax].hll_item.cmd == HLL_REPEAT )
                ...
                ReplaceLabel(buffer, olabel, nlabel)
                mov r8,GetLabelStr(nlabel, &buff)
                tsprintf(rbx, "%s%s%s", r8, LABELQUAL, &EOLSTR)
            .else
                mov r8,GetLabelStr(olabel, &buff)
                tsprintf(rbx, "%s%s%s", r8, LABELQUAL, &EOLSTR)
...
ExpandCStrings proc fastcall public uses rdi rbx tokenarray:ptr asm_tok
...
GetProcVtbl proc private sym:ptr asym, name:ptr sbyte
    ...
    .if ( [rcx].asym.flag2 & S_VTABLE )
        SearchNameInStruct( [rcx].asym.vtable, name, 0, 0 )
    .endif
    ret

GetProcVtbl endp

GetProc proc private uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok, opnd:ptr expr
    ...
    .return rcx .if dl == MT_PTR && rcx && [rcx].asym.flag1 & S_ISPROC
...
GetParamId proc private uses rsi rdi id:int_t, sym:asym_t
...
GetMacroReturn proc private uses rsi rbx i:int_t, tokenarray:ptr asm_tok
  ...
  local opnd:expr
    ...
    .if ( [rax].asym.state == SYM_STRUCT_FIELD )
        ...
        strcpy( &mac_name, [rcx].asym.name )
        strcat( rax, "_" )
        strcat( rax, [rsi].asym.name )
...
StripSource proc private uses rsi rdi rbx i:uint_t, e:uint_t, tokenarray:ptr asm_tok

  local sym:dsym_t
  local mac:string_t
  local curr:asym_t
  local proc_id:ptr asm_tok ; foo( 1, bar(...), ...)
  local parg_id:int_t ; foo.paralist[1] = return type[al|ax|eax|[rdx::eax|rax|xmm0]]
  local opnd:expr
  local bracket:int_t
  local reg2:int_t
        ...
        .if !proc_id && eax > 1
            ...
            .if [rbx+asm_tok].token == T_COMMA && [rbx].token != T_CL_SQ_BRACKET
                ...
                .elseif SymFind( [rbx].string_ptr )
                    ...
                    .if ( ModuleInfo.Ofssize == USE64 && \
                        ( [rax].asym.mem_type == MT_REAL4 || \
                          [rax].asym.mem_type == MT_REAL8 ) )
                        mov eax,16
                    .else
...
macro_args:
    ...
    strcpy(ModuleInfo.currsource, rsi)
...
LKRenderHllProc proc private uses rsi rdi rbx dst:string_t, i:uint_t, tokenarray:ptr asm_tok
  ...
  local vparray:asym_t
    ...
    .for ( : [rbx].token != T_FINAL : rbx+=asm_tok, j++ )
        ...
        .endif
    .endf
    .return asmerr(2008, [rbx].string_ptr) .if [rbx].token != T_OP_BRACKET
    ...
    .if ( [rbx].token == T_OP_SQ_BRACKET )
        ...
        .if dotcount
            ...
            .elseif [rdx+asm_tok*2].asm_tok.token == T_OP_BRACKET
                ...
                div ecx
                mov r8d,eax
                .ifd EvalOperand( &brcount, tokenarray, r8d, &opnd, 0 ) != ERROR
                    mov rax,opnd.type
                .else
                    xor eax,eax
                .endif
    ...
    .else
        ...
        .elseif rax && [rax].asym.state == SYM_TYPE && \
    ...
    .if ( rdx && [rdx].asym.state == SYM_TYPE && [rdx].asym.typekind == TYPE_STRUCT )
        ...
        .endif
        .if ( method == NULL )
            .if ( strcmp( [rsi].asym.name, name ) == 0 )
                inc vtable
                inc constructor
            .endif
        .elseif ( vparray )
        ...
        .if ( [rbx].token == T_OP_SQ_BRACKET )
            ...
            .else
                ...
                .for ( rsi = &[rbx+asm_tok] : rsi < sqbrend : rsi += asm_tok )
                    strcat( rax, [rsi].asm_tok.string_ptr )
                .endf
            .endif
    ...
    .for ( brcount = 1 : [rbx].token != T_FINAL : )
        ...
        .if al == '&'
            ...
            mov ecx,5
        .elseif al == T_COMMA && \
                [rbx+asm_tok].token == T_CL_BRACKET && brcount == 1
            inc j
            add rbx,asm_tok
            .break
        .else
...
RenderHllProc proc fastcall private uses rsi rdi dst:string_t, i:uint_t, tokenarray:ptr asm_tok
  ...
  local oldstat:input_status
    ...
    strcpy( rax, ModuleInfo.currsource )
...
ExpandHllProc proc fastcall public uses rsi rdi dst:string_t, i:int_t, tokenarray:ptr asm_tok
...
ExpandHllProcEx proc fastcall public  uses rsi rdi buffer:string_t, i:int_t, tokenarray:ptr asm_tok
...
EvaluateHllExpression proc fastcall public uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok,
  ...
  local hllop:hll_opnd, b[MAX_LINE_LEN]:char_t
    ...
    .if ( ModuleInfo.strict_masm_compat == 0 ) && !eax && [rbx].asm_tok.hll_flags & T_HLL_DELAY
        ...
        .while edi < ModuleInfo.token_count
            ...
            .if [rbx+rax].asm_tok.hll_flags & T_HLL_MACRO
                ...
                strcpy( buffer, [rbx].asm_tok.tokpos )
                mov rax,hll
    ...
    .ifd ExpandHllProc( rdi, [rsi], rbx ) != ERROR
        ...
        .ifd GetExpression( hll, rsi, rbx, ilabel, is_true, rcx, &hllop ) != ERROR
            ...
            .return asmerr(2154) .if [rbx].asm_tok.token != T_FINAL
...
ExpandHllExpression proc fastcall public uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok,
        ilabel:int_t, is_true:int_t, buffer:string_t
  ...
  local rc:DWORD,
    ...
    strcpy(ModuleInfo.currsource, rdi)
...
CheckCXZLines proc private uses rsi rdi rbx p:string_t
    ;
...
RenderUntilXX proc private uses rdi hll:ptr hll_item, cmd:uint_t
...
GetJumpString proc private cmd:uint_t
...
HllStartDir proc uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
    ...
    .switch eax
      ...
      .case T_DOT_IFA .. T_DOT_IFNZ
        ...
        .if GetJumpString(cmd)
            ...
            strcat(strcpy(&buffer, rax), " ")
            lea rcx,buff
            AddLineQueue(strcat(rax, rcx))
        .endif
        .endc
...
;
HllEndDir proc uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
  ...
  local rc:int_t, cmd:int_t, buffer[MAX_LINE_LEN]:char_t
    ...
    .switch eax
      ...
      .case T_DOT_UNTILCXZ
        ...
        .return asmerr(1010, [rbx+rdx].string_ptr) .if ecx != HLL_REPEAT
...
HllContinueIf proc uses rsi rdi rbx hll:ptr hll_item, i:ptr int_t, tokenarray:ptr asm_tok,
        ...
        .if [rbx].token == T_DIRECTIVE
            ...
            .switch eax
              ...
              .case T_DOT_IFA .. T_DOT_IFNZ
                ...
                strcpy(rdi, GetJumpString( [rbx].tokval))
                strcat(rdi, " ")
                strcat(rdi, &buff)
                .if is_true
                    InvertJump(rdi)
...
HllExitDir proc USES rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
  ...
  local rc:     int_t,
    ...
    .switch eax
      ...
      .case T_DOT_ELSE
        ...
        .return asmerr(1010, [rbx].string_ptr) .if ( [rsi].cmd != HLL_IF )
        ;
      ...
      .case T_DOT_CONTINUE
        ...
        .if [rbx+asm_tok].token == T_OP_BRACKET && [rbx+asm_tok*3].token == T_CL_BRACKET
            ...
            .if cmd == T_DOT_CONTINUE
                ...
                mov rax,[rbx+asm_tok*2].string_ptr
                .if byte ptr [rax] == '0'
            ...
            mov ecx,atol([rbx+asm_tok*2].string_ptr)
            add i,3
            add rbx,asm_tok*3
            mov eax,cmd
        .endif
