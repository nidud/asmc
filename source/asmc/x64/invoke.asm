...
PushInvokeParam proc private uses rsi rdi rbx r12 r13 i:int_t, tokenarray:ptr asm_tok, pproc:ptr dsym,
  ...
  local ParamId:int_t
    ...
    .for ( ecx = 0 : ecx <= reqParam : )
        ...
        .return(ERROR) .if ( [rbx].token == T_FINAL ) ;; this is no real error!
    ...
    .for ( rdx = rbx: [rdx].asm_tok.token != T_COMMA && [rdx].asm_tok.token != T_FINAL: rdx+=asm_tok )
    .endf
    ...
    .if ( t_addr )
        ...
        .return .ifd ( EvalOperand( &j, tokenarray, Token_Count, &opnd, ModuleInfo.invoke_exprparm ) == ERROR )
        ...
        .if ( fastcall_id  )
            ...
            add r10,rcx
            .if ( [r10].fastcall_conv.handleparam( pproc,
...
ifndef ASMC64
    ...
    .if ( fastcall_id )
        ...
        lea r10,fastcall_tab
        add r10,rcx
        .if ( [r10].fastcall_conv.handleparam( pproc, reqParam, curr, t_addr, &opnd, &fullparam, r0flags ) )
...
InvokeDirective proc uses rsi rdi rbx r12 r13 i:int_t, tokenarray:ptr asm_tok
  ...
  local struct_ptr:ptr asm_tok
    ...
    .if ( fastcall_id )
        ...
        lea r10,fastcall_tab
        add r10,rcx
        mov porder,[r10].fastcall_conv.invokestart(
                   rsi, numParam, i, tokenarray, &value )
    .endif
    ...
    mov rcx,pproc
    .if ( !pmacro && \
          [rcx].asym.langtype == LANG_SYSCALL && \
          [rdx].proc_info.flags & PROC_HAS_VARARG && \
            ...
            .if ( fastcall_id )
                ...
                .for ( ebx = 0: ebx < cnt: ebx++ )
                    ...
                    .elseif ( [rdi].mem_type == MT_ABS )
                        ...
                        mov [rdi].name,&@CStr("")
                    .elseif ( reg )
                        mov args[rbx*8],get_regname(reg, size)
                    .else
                        mov args[rbx*8],LclAlloc(16)
                        movzx eax,ModuleInfo.Ofssize
                        shl eax,2
                        .if ( ModuleInfo.Ofssize == USE64 )
                            tsprintf( args[rbx*8], "[rsp+%d*%d]", ebx, eax )
                        .else
                            mov ecx,ebx
                            sub ecx,stk
                            tsprintf( args[rbx*8], "[esp+%d*%d]", ecx, eax )
                        .endif
                    .endif
                .endf
            .endif
...
ifndef ASMC64
            .elseif ( ModuleInfo.Ofssize == USE32 )
                ...
                .elseif ( [rbx+asm_tok].tokval != T_EAX )
                    ...
                    .if ( [rbx+asm_tok].token == T_REG && \
                          [rbx+asm_tok].tokval > T_EAX && \
                          [rbx+asm_tok].tokval <= T_EDI )
                        mov reg,[rbx+asm_tok].tokval
                    .elseif ( [rbx+asm_tok].token == T_RES_ID && \
                              [rbx+asm_tok].tokval == T_ADDR )
    ...
    .elseif ( fastcall_id )
        ...
        lea r10,fastcall_tab
        add r10,rcx
        [r10].fastcall_conv.invokeend( pproc, numParam, value )
    .endif
