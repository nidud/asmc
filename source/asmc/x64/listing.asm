...
LstWrite proc __ccall uses rsi rdi rbx type:lsttype, oldofs:uint_t, value:ptr
  ...
  local ll:lstleft
    ...
    .if ( ModuleInfo.list == FALSE || CurrFile[LST*8] == NULL  || ( ModuleInfo.line_flags & LOF_LISTED ) )
        .return
    .endif
...
if USELSLINE
            ...
            .endif
        .endif
        fseek( CurrFile[LST*8], list_pos, SEEK_SET )
    .endif
    ...
    .case LSTTYPE_CODE
        ...
        .if ( [rdx].seg_info.CodeBuffer == NULL || \
              [rdx].seg_info.written == FALSE )
    ...
    .endif
    fwrite( &ll.buffer, 1, idx, CurrFile[LST*8] )
    ...
    .if ( Parse_Pass == PASS_1 || UseSavedState == FALSE )
        .if ( len )
            fwrite( pSrcline, 1, len, CurrFile[LST*8] )
        .endif
        .if ( len2 )
            fwrite( ModuleInfo.CurrComment, 1, len2, CurrFile[LST*8] )
        .endif
        fwrite( NLSTR, 1, NLSIZ, CurrFile[LST*8] )
    .endif
    ...
    .for ( rbx = ll.next: rbx: rbx = [rbx].lstleft.next )
        fwrite( &[rbx].lstleft.buffer, 1, 32, CurrFile[LST*8] )
        fwrite( NLSTR, 1, NLSIZ, CurrFile[LST*8] )
...
LstPrintf proc __ccall format:string_t, args:vararg
    ...
    .if ( CurrFile[LST*8] )
        ...
        add list_pos,tvfprintf( CurrFile[LST*8], format, &args )
    .endif
    ret
...
get_seg_align proc __ccall s:ptr seg_info, buffer:string_t
    ...
    lea r8,strings
    .switch( ecx )
    .case 0: .return( [r8+LS_BYTE]  )
    .case 1: .return( [r8+LS_WORD]  )
    .case 2: .return( [r8+LS_DWORD] )
    .case 3: .return( [r8+LS_QWORD] )
    .case 4: .return( [r8+LS_PARA]  )
    .case 8: .return( [r8+LS_PAGE]  )
    .case MAX_SEGALIGNMENT: .return( [r8+LS_ABS] )
    .default
        mov eax,1
        shl eax,cl
...
get_seg_combine proc __ccall s:ptr seg_info

    lea r8,strings
    .switch( [rcx].seg_info.combine )
    .case COMB_INVALID:    .return( [r8+LS_PRIVATE] )
    .case COMB_STACK:      .return( [r8+LS_STACK]   )
    .case COMB_ADDOFF:     .return( [r8+LS_PUBLIC]  )
    ; v2.06: added
    .case COMB_COMMON:     .return( [r8+LS_COMMON]  )
    .endsw
    .return( "?" )

get_seg_combine endp

log_macro proc __ccall sym:ptr asym
...
SimpleTypeString proc __ccall mem_type:byte
    ...
    lea r8,strings
    .switch ( ecx )
    .case 1: .return( [r8+LS_BYTE] )
    .case 2: .return( [r8+LS_WORD] )
    .case 4: .return( [r8+LS_DWORD] )
    .case 6: .return( [r8+LS_FWORD] )
    .case 8: .return( [r8+LS_QWORD] )
    .case 10:.return( [r8+LS_TBYTE] )
    .case 16:.return( [r8+LS_OWORD] )
    .case 32:.return( [r8+LS_YWORD] )
    .case 64:.return( [r8+LS_ZWORD] )
    .endsw
    .return( "" )
  ...
  local mem_type:byte
    ...
    .case MT_PTR
        ...
        .else
            ...
            .if ( [rsi].asym.is_far )
                mov rdi,[r8+LS_FAR16+rcx*8]
            .else
                mov rdi,[r8+LS_NEAR16+rcx*8]
            .endif
        .endif
        ...
        .if ( buffer ) ; Currently, 'buffer' is only != NULL for typedefs
            ...
            .for ( i = [rsi].asym.is_ptr: i: i-- )
                lea r8,strings
                add rbx,tsprintf( rbx, "%s %s ", rdi, string_t ptr [r8+LS_PTR] )
            .endf
...
GetLanguage proc __ccall sym:ptr asym

    movzx eax,[rcx].asym.langtype
    .if ( eax <= LANG_WATCALL )
        lea rdx,strings
        .return( [rdx+rax*8 + LS_VOID] )
    .endif
    .return( "?" )
...
log_group proc __ccall uses rsi rdi grp:ptr asym, segs:ptr dsym
    ...
    lea rdx,strings
    mov r9,[rdx+LS_GROUP]
    LstPrintf( "%s %s        %s", [rsi].asym.name, r8, r9 )
    LstNL()
...
get_proc_type proc __ccall sym:ptr asym
...
get_sym_seg_name proc __ccall sym:ptr asym
...
log_proc proc __ccall uses rsi rdi rbx r12 sym:ptr asym
  ...
  local buffer[MAX_ID_LEN+1]:char_t
    ...
    .if ( [rsi].asym.state == SYM_EXTERNAL && [rsi].asym.altname )
        ...
        mov r12,get_proc_type( rbx )
        get_sym_seg_name( rbx )
        mov rdx,pdots
        add rdx,2
        LstPrintf( rdi, [rbx].asym.name, rdx, r12, [rbx].asym.offs, rax )
        LstNL()
    .endif
    ...
    .if ( [rsi].asym.state == SYM_INTERNAL )
        ...
        .if ( eax == LANG_STDCALL || eax == LANG_C || eax == LANG_SYSCALL || eax == LANG_VECTORCALL || \
            ...
            .for ( : cnt: cnt-- )
                ...
                .if ( [rdi].asym.state == SYM_TMACRO )
                    mov r9,GetMemtypeString( rdi, NULL )
                    LstPrintf( "  %s %s        %-17s %s",
                        [rdi].asym.name, pdots, r9, [rdi].asym.string_ptr )
                .else
                    mov rcx,[rsi].dsym.procinfo
                    mov r12,GetResWName( [rcx].proc_info.basereg, NULL )
                    mov rcx,GetMemtypeString( rdi, NULL )
                    .if ( [rdi].asym.sflags & S_ISVARARG )
                        lea rcx,strings
                        mov rcx,[rcx+LS_VARARG]
                    .endif
                    LstPrintf( szFmtProcStk, [rdi].asym.name, pdots, rcx, r12, '+', [rdi].asym.offs )
                .endif
                LstNL()
            .endf
        .else
            ...
            .for ( rdi = [rdi].proc_info.paralist: rdi : rdi = [rdi].dsym.nextparam )
                ...
                mov r9,GetMemtypeString( rdi, NULL )
                LstPrintf( szFmtProcStk, [rdi].asym.name, pdots, r9,
                          rbx, '+', [rdi].asym.offs )
                LstNL()
            .endf
        .endif
        ...
        .for ( rdi = [rdi].proc_info.labellist: rdi : rdi = [rdi].dsym.nextll )
            ...
            .for ( rbx = rdi: rbx: rbx = [rbx].asym.nextitem )
                ...
                mov r12,get_proc_type( rbx )
                get_sym_seg_name( rbx )
                LstPrintf( p, [rbx].asym.name, pdots, r12, [rbx].asym.offs, eax )
                LstNL()
            .endf
        .endf
    .endif
...
log_symbol proc __ccall uses rsi rdi rbx sym:ptr asym
  ...
  local pdots:string_t
    ...
    .case SYM_TMACRO
        lea rdx,strings
        mov r8,[rdx+LS_TEXT]
        LstPrintf( "%s %s        %s   %s", [rdi].asym.name, pdots, r8, [rdi].asym.string_ptr )
        LstNL()
        .endc
    .case SYM_ALIAS
        ...
        lea rdx,strings
        mov r8,[rdx+LS_ALIAS]
        LstPrintf( "%s %s        %s  %s", [rdi].asym.name, pdots, r8, [rcx].asym.name )
        LstNL()
        .endc
    .endsw
    ret
...
; write symbol table listing
    ...
    assume r12:ptr print_item

LstWriteCRef proc __ccall uses rsi rdi rbx r12
  ...
  local queues[LQ_LAST]:qdesc
    ...
    .if ( CurrFile[LST*8] == NULL || Options.no_symbol_listing == TRUE )
        .return
    .endif

    ; go to EOF

    fseek( CurrFile[LST*8], 0, SEEK_END )
    ...
    .for ( ebx = 0: ebx < lengthof(cr): ebx++ )
        ...
        .if ( [rax+rcx].qdesc.head )
            .if ( [r12+rdi].capitems )
                .for ( rsi = [r12+rdi].capitems: word ptr [rsi]: rsi += 2 )
                    mov eax,2
                    .if ( rsi != [r12+rdi].capitems )
                        xor eax,eax
                    .endif
            ...
            .for ( rsi = [rax+rcx].qdesc.head: rsi : rsi = [rsi].dsym.next )
                xor edx,edx
                .if ( [r12+rdi].print_item.flags & PRF_ADDSEG )
                    mov rdx,queues[LQ_SEGS*qdesc].head
                .endif
                [r12+rdi].function( rsi, rdx, 0 )
            .endf
        .endif
    .endf
