...
ConstructorCall proc __ccall private uses rsi rdi rbx r12 \
...
AssignStruct proc __ccall private uses rsi rdi rbx r12 r13 r14 name:string_t, sym:asym_t, string:string_t
    ...
    .while 1
        ...
        .else
            ...
            .if val
                ...
                .if val == '"' || ( val == 'L' &&  val[1] == '"')
                    .if array
                        AddLineQueueX( " mov %s[%d], &@CStr(%s)", r12, ecx, &val )
                    .else
                        AddLineQueueX( " mov %s.%s, &@CStr(%s)", r12, [rbx].sfield.name, &val )
                    .endif
                .elseif array
                    AddLineQueueX( " mov %s[%d], %s", r12, ecx, &val )
                .else
                    AssignString( r12, rbx, &val )
                .endif
            .endif
        .endif
...
AssignId proc __ccall private uses rsi rdi rbx name:string_t, sym:asym_t, type:asym_t, string:string_t
  ...
  local Id[128]:char_t, size:int_t, f:sfield
    ...
    .if ( r8 == NULL )
       ...
       .return AssignStruct( name, NULL, r9 )
    .endif

    mov rdi,rdx
    .if !( [rdi].asym.flag1 & S_ISARRAY )

        .return AssignStruct( rcx, type, r9 )
    .endif
...
ClearStruct proc __ccall uses rsi rdi rbx r12 name:string_t, sym:asym_t
    ...
    .if ModuleInfo.Ofssize == USE64
        ...
        .else
            ...
            .for ( ebx = 0 : edi >= 8 : edi -= 8, ebx += 8 )
                ...
                AddLineQueueX( " mov qword ptr %s[%d], rax", r12, ebx )
            .endf

            .if ( edi >= 4 )

                AddLineQueueX( " mov dword ptr %s[%d], eax", r12, ebx )
                sub edi,4
                add ebx,4
            .endif
            ...
            .for ( : edi : edi--, ebx++ )
                ...
                AddLineQueueX( " mov byte ptr %s[%d], al", r12, ebx )
            .endf
        .endif
...
AssignValue proc __ccall private uses rsi rdi rbx name:string_t, i:int_t,
  ...
  local opndx:expr
    ...
    .if [rbx].token == T_STRING && [rbx].bytval == '{'
        ...
        SymSearch( name )
        .return asmerr( 2008, [rbx].tokpos ) .if !rax
        ...
        .elseifd ( EvalOperand( &i, tokenarray, Token_Count, &opndx, 0 ) != ERROR )
            ...
            .elseif ( opndx.kind == EXPR_FLOAT )
                ...
                AddLineQueueX(
                    ...
                    " mov dword ptr %s[4], HIGH32(%s)", name, [rbx].tokpos, name, [rbx].tokpos )
        ...
        .if [rbx].token != T_FINAL
            ...
            .elseif [rbx].token == T_OP_BRACKET
                ...
                .for eax = 1 : [rbx].token != T_FINAL : rbx += asm_tok
                    ...
                    .endif
                .endf
                .return asmerr( 2045 ) .if [rbx].token != T_CL_BRACKET
