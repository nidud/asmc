...
PutData proc __ccall private uses rsi rdi objr:ptr omf_rec, pdata:ptr byte, len:int_t
...
AttachData proc __ccall private objr:ptr omf_rec, pdata:ptr byte, len:size_t
...
omf_GetGrpIdx proc __ccall sym:ptr asym
...
omf_OutSelect proc __ccall uses rsi rdi rbx is_data:int_t
ifndef ASMC64
...
omf_write_linnum proc __ccall private uses rsi rdi rbx is32:byte
...
omf_write_fixupp proc __ccall private uses rsi rdi rbx s:ptr dsym, is32:char_t
...
get_omfalign proto __ccall private alignment:byte

;; write an LEDATA record, optionally write fixups

omf_write_ledata proc __ccall private uses rsi rdi rbx s:ptr dsym
  ...
  local size:int_t
    ...
    mov eax,[rsi].seg_info.current_loc
    sub eax,[rsi].seg_info.start_loc
    mov size,eax
    ...
    .ifs ( eax > 0 && write_to_file == TRUE )
        ...
        .if ( [rsi].seg_info.comdatselection )
            ...
            AttachData( &obj, [rsi].seg_info.CodeBuffer, size )
...
omf_write_theadr proc __ccall private name:string_t
...
omf_check_flush proc __ccall uses rsi rdi rbx curr:ptr line_num_info
...
ifndef ASMC64
omf_end_of_pass1 proc __ccall private
...
ifndef ASMC64
omf_write_dosseg proc __ccall private
...
omf_write_lib proc __ccall private uses rsi rdi rbx
...
omf_write_export proc __ccall private uses rsi rdi rbx
...
omf_write_grpdef proc __ccall private uses rsi rdi rbx
...
get_omfalign proc __ccall private alignment:byte
...
omf_write_segdef proc __ccall private uses rsi rdi rbx
...
omf_write_lnames proc __ccall private uses rsi rdi rbx
...
GetExt proc __ccall private r:ptr readext
...
omf_write_extdef proc __ccall private uses rsi rdi rbx
...
get_size_of_comdef_number proc __ccall private value:dword
...
put_comdef_number proc __ccall private uses rsi rdi rbx buffer:ptr byte, value:dword
...
omf_write_autodep proc __ccall private uses rsi rdi rbx
...
omf_write_alias proc __ccall private uses rsi rdi rbx
...
omf_write_pubdef proc __ccall private uses rsi rdi rbx
...
omf_write_header_dbgcv proc __ccall private uses rbx r12
  ...
  local obj:omf_rec
    ...
    .for ( r12 = &SymDebParm, ebx = 0: ebx < DBGS_MAX: ebx++, r12+=dbg_section )

        CreateIntSegment( [r12].dbg_section.name, [r12].dbg_section.cname, 0, USE32, TRUE )
...
omf_write_debug_tables proc __ccall private uses rsi rdi rbx

    .if ( SymDebSeg[DBGS_SYMBOLS*8] && SymDebSeg[DBGS_TYPES*8] )

        mov rax,SymDebSeg[DBGS_SYMBOLS*8]
        mov rcx,[rax].dsym.seginfo
        mov [rcx].seg_info.CodeBuffer,CurrSource

        mov rdx,SymDebSeg[DBGS_TYPES*8]
        mov rcx,[rdx].dsym.seginfo
        add rax,1024
        mov [rcx].seg_info.CodeBuffer,rax

        cv_write_debug_tables( SymDebSeg[DBGS_SYMBOLS*8], SymDebSeg[DBGS_TYPES*8], NULL )
    .endif
    ret
...
if TRUNCATE
    ...
    mov size,ftell( CurrFile[OBJ*8] )
    mov fh,_fileno( CurrFile[OBJ*8] )
    _chsize( fh, size )
endif
    ...
    fseek( CurrFile[OBJ*8] , seg_pos, SEEK_SET )
    omf_write_segdef()
    ...
    fseek( CurrFile[OBJ*8], public_pos, SEEK_SET)
    omf_write_pubdef()
   .return( NOT_ERROR )
...
omf_write_header_initial proc __ccall private modinfo:ptr module_info
  ...
  local ext_idx:uint_16
    ...
    omf_write_theadr( CurrFName[ASM*8] ) ; write THEADR record, main src filename
    ...
    mov seg_pos,ftell( CurrFile[OBJ*8] )
    omf_write_segdef()
    ...
    mov public_pos,ftell( CurrFile[OBJ*8] )
    omf_write_pubdef()
...
omf_init proc __ccall modinfo:ptr module_info

ifndef ASMC64
    mov [rcx].module_info.WriteModule,&omf_write_module
    mov [rcx].module_info.Pass1Checks,&omf_write_header_initial
    mov SymDebSeg[DBGS_SYMBOLS*8],NULL
    mov SymDebSeg[DBGS_TYPES*8],NULL
if MULTIHDR
