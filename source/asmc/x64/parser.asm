...
sym_add_table proc __ccall queue:ptr symbol_queue, item:dsym_t
...
sym_remove_table proc __ccall queue:ptr symbol_queue, item:dsym_t
...
SizeFromRegister proc __ccall registertoken:int_t
...
SizeFromMemtype proc __ccall mem_type:uchar_t, Ofssize:int_t, type:asym_t
...
MemtypeFromSize proc __ccall size:int_t, ptype:ptr byte

    UNREFERENCED_PARAMETER(size)
    UNREFERENCED_PARAMETER(ptype)

    lea r8,SpecialTable
    add r8,T_BYTE * special_item
    .for ( : [r8].special_item.type == RWT_STYPE : r8 += special_item )

        mov al,[r8].special_item.bytval
        and eax,MT_SPECIAL
        .ifz
...
OperandSize proc __ccall opnd:int_t, CodeInfo:ptr code_info
...
comp_mem16 proc __ccall reg1:int_t, reg2:int_t
...
check_assume proc __ccall CodeInfo:ptr code_info, sym:asym_t, default_reg:int_t
  ...
  local assum:asym_t
    ...
    .return .if rdx && [rdx].state == SYM_UNDEFINED

    mov reg,GetAssume( SegOverride, rdx, r8d, &assum )
...
set_frame proc __ccall public sym:asym_t
...
set_frame2 proc __ccall public sym:asym_t
...
ifndef ASMC64
        .if ( [rsi].Ofssize == USE16 && [rsi].adrsiz == 0 ) || \
            ...
            .return asmerr(2011) .if !InWordRange([rsi].opnd[rbx].data32l)
...
segm_override proc __ccall public uses rsi rdi rbx opndx:expr_t, CodeInfo:ptr code_info
    ...
    .if rdi
        .if [rdi].token == T_REG
            ...
            .return asmerr(2108) .if [rcx+rax].assume_info.error

            ; ES,CS,SS and DS overrides are invalid in 64-bit

            .return asmerr(2202) .if rsi && [rsi].Ofssize == USE64 && ebx < ASSUME_FS
...
idata_nofixup proc __ccall private uses rsi rdi rbx CodeInfo:ptr code_info, CurrOpnd:uint_t, opndx:expr_t
   ...
   .new size:int_t = 0
    ...
    .if IS_ANY_BRANCH( [rsi].token )
        .return( process_branch( rcx, edx, r8 ) )
    .endif
    ...
    .if [rdi].hlvalue ; magnitude > 64 bits?
    ...
    .if ( size || [rsi].Ofssize == USE64 && [rsi].token == T_MOV &&
        ...
        mov [rsi].opnd[rbx].data32h,edx
       .return NOT_ERROR
    .endif

    .ifs ( rax <= minintvalues || rax > maxintvalues )
...
idata_fixup proc __ccall public uses rsi rdi rbx CodeInfo:ptr code_info, CurrOpnd:uint_t, opndx:expr_t
    ...
    .if IS_ANY_BRANCH( [rsi].token )
        .return process_branch( rsi, edx, r8 )
    .endif
    ...
    .return asmerr(2070) .if [rdi].inst == T_SHORT
    ...
    .case 1
        ...
        mov [rsi].opsiz,FALSE ;; v2.10: reset opsize is not really a good idea - might have been set by previous operand
        .endc
    ...
    .elseif r12
        ...
        .if [r12].asym.type
            ...
            .if ebx == 4 && al != [rsi].Ofssize
                mov [rdi].Ofssize,al
            .endif
        .elseif [r12].asym.mem_type == MT_PTR

            mov ecx,MT_NEAR
            .if [r12].asym.is_far
                mov ecx,MT_FAR
            .endif
            mov ebx,SizeFromMemtype( cl, [r12].asym.Ofssize, NULL )
            and [rsi].flags,not CI_ISFAR
            .if [r12].asym.is_far
                or [rsi].flags,CI_ISFAR
            .endif
        .else
            .if [r12].asym.flag1 & S_ISARRAY
                mov ecx,[r12].asym.total_length
                mov eax,[r12].asym.total_size
                xor edx,edx
                div ecx
                mov ebx,eax
            .else
                mov ebx,[r12].asym.total_size
            .endif
        .endif
    .else
...
memory_operand proc __ccall uses rsi rdi rbx r12 r13 r14 r15 CodeInfo:ptr code_info,
  ...
  local mem_type:byte
    ...
    .elseif ah == MT_ADDRESS
        .if [rdi].Ofssize == USE_EMPTY && r12
            mov [rdi].Ofssize,GetSymOfssize(r12)
        .endif
    ...
    .case T_CALL
        ...
        .if [rsi].mem_type == MT_EMPTY
            ...
            .if ( ModuleInfo.m510 == FALSE &&
                  Parse_Pass > PASS_1 && [rdi].sym == NULL )
                .return asmerr(2023)
            .endif
            mov eax,MT_WORD
            .if [rsi].Ofssize == USE64
    ...
    .elseif [rsi].mem_type == MT_EMPTY
        ...
        .case T_DEC
            ...
            .return asmerr(2023) .if [rdi].sym == NULL
            .endc
        .case T_PUSH
        .case T_POP
    ...
    .if ( index != EMPTY )
        ...
        .if ( ( cl == USE16 && [rsi].adrsiz == 1 ) || cl == USE64  ||
            ...
            .if eax >= P_386
                ...
                .case 0
                .case 1: .endc                          ; s = 00
                .case 2: mov r13b,SCALE_FACTOR_2: .endc ; s = 01
                .case 4: mov r13b,SCALE_FACTOR_4: .endc ; s = 10
                .case 8: mov r13b,SCALE_FACTOR_8: .endc ; s = 11
        ...
        .else
            ...
            .return asmerr(2032) .if [rdi].scale
        .endif
    .endif
    ...
    .if with_fixup
        ...
        .if [rdi].flags & E_IS_ABS
            ...
            .endif
        .elseif r12
            mov r15b,GetSymOfssize(r12)
        .elseif SegOverride
            mov r15b,GetSymOfssize(SegOverride)
        .else
            mov r15b,[rsi].Ofssize
        .endif
        ...
        .if ( base == EMPTY && index == EMPTY )
            ...
            mov eax,r15d
            mov [rsi].adrsiz,ADDRSIZE( [rsi].Ofssize, eax )
            .if r15b == USE64
        ...
        .if r14d == FIX_OFF32
            .if [rdi].inst == T_IMAGEREL
                mov r14d,FIX_OFF32_IMGREL
            .elseif [rdi].inst == T_SECTIONREL
                mov r14d,FIX_OFF32_SECREL
            .endif
        .endif
        ...
        .if ( [rsi].token != T_XLAT && [rsi].token != T_XLATB )
            CreateFixup(r12, r14d, OPTJ_NONE)
            mov [rsi].opnd[rbx].InsFixup,rax
        .endif

    .endif
    .return .ifd set_rm_sib(rsi, CurrOpnd, r13b, index, base, r12) == ERROR
...
process_address proc __ccall uses rsi rdi rbx CodeInfo:ptr code_info,
    ...
    .if [rdi].flags & E_INDIRECT ;; indirect register operand or stack var
        ...
        .if [rdi].hvalue && ( [rdi].hvalue != -1 || [rdi].value >= 0 )
            .return EmitConstError(rdi) .if ModuleInfo.Ofssize == USE64
        ...
        .default
            ...
            .if ebx == OPND2 && eax == 0
                ...
                .repeat
                    ...
                    imul eax,eax,opnd_class
                    lea r8,opnd_clstab
                    .if [r8+rax].opnd_class.opnd_type[4] & OP_I
                        .return idata_fixup(rsi, ebx, rdi)
                    .endif
                    add rcx,instr_item
...
process_const proc __ccall CodeInfo:ptr code_info, CurrOpnd:uint_t, opndx:expr_t
    ...
    .if [r8].expr.mem_type & MT_FLOAT
        mov [r8].expr.float_tok,NULL
    .endif
    mov rax,[r8].expr.quoted_string
    .return asmerr(2047) .if rax && [rax].asm_tok.stringlen == 0
    ...
    mov r9,[rcx].code_info.pinstr
    mov al,[r9].instr_item.opcode
    and eax,0xf7

   .return( NOT_ERROR ) .if ( eax == 0xc2 && edx == OPND1 && [r8].expr.value == 0 )
    idata_nofixup(rcx, edx, r8)
    ret
  ...
  local flags:uint_t
    ...
    .if eax & OP_R8
        ...
        .if [rsi].Ofssize == USE64 && regno >=4 && regno <= 7
            ...
            lea r8,SpecialTable
            .if [r8+rax].special_item.cpu == P_86
        ...
        .if regtok >= T_AH && regtok <= T_BH
            mov ecx,RH_ERROR
        .endif
        lea r8,StdAssumeTable
        .return asmerr(2108) .if [r8+rax].assume_info.error & cl
    ...
    .elseif eax & OP_R ;; 16-, 32- or 64-bit GPR?
        ...
        and al,OP_R
        lea r8,StdAssumeTable
        .return asmerr(2108) .if [r8+rcx].assume_info.error & al
    ...
    .elseif eax & OP_SR
        .if regno == 1 ;; 1 is CS
            ...
            .return asmerr(2008, "POP CS") .if [rsi].token == T_POP
        .endif
    .elseif eax & OP_ST
    ...
    .elseif eax & OP_RSPEC ;; CRx, DRx, TRx
        .if [rsi].token != T_MOV
            .return asmerr(2151) .if [rsi].token == T_PUSH
            .return asmerr(2070)
        .endif
...
HandleStringInstructions proc __ccall uses rsi rdi rbx CodeInfo:ptr code_info, opndx:expr_t
  ...
  local op_size:int_t
    ...
    .if [r8].opnd_class.opnd_type[rcx] == OP_NONE
        and [rsi].flags,not CI_ISWIDE
        mov [rsi].opsiz,FALSE
    .endif
    ...
    .if [r8].opnd_class.opnd_type[rcx] != OP_NONE && [rsi].opnd[rbx].type != OP_NONE
...
; tofix: add a flag in instr_table[] if there's NO check to be done.
  ...
  local op2_size:int_t
    ...
    .case T_VCVTTPD2DQ
    .case T_VCVTPD2PS
        .return asmerr(2023) .if edx == OP_M && ( [rdi+expr].flags & E_INDIRECT )
        .endc
    .case T_VMOVDDUP
        .endc .if !( ecx & OP_YMM )
        ; fall through
    ...
    def_check:
        ...
        .if op1_size == 1 && op2 == OP_I16 && [rsi].opnd[OPNI2].data32l <= UCHAR_MAX && \
            [rsi].opnd[OPNI2].data32l >= -255
...
IsType proc __ccall name:string_t

    UNREFERENCED_PARAMETER(name)

    SymSearch(rcx)

   .return .if ( rax && [rax].asym.state == SYM_TYPE )
   .return ( NULL )
...
; Zero Mask                  {k1}{z}
;
parsevex proc __ccall string:string_t, result:ptr uchar_t
...
ParseLine proc __ccall uses rsi rdi rbx tokenarray:token_t
  ...
  local buffer:ptr char_t
    ...
    mov rbx,rcx
    .return EnumDirective(0, rbx) .if ( CurrEnum && [rbx].token == T_ID )
...
continue:
    ...
    .if j || [rsi].token != T_INSTRUCTION
        ...
        .elseif ( i != 0 || [rbx].dirtype != '{' )
            ...
            .return EnumDirective(0, rbx) .if ( CurrEnum && [rbx].token == T_STRING )
    ...
    .if ( [rsi].tokval >= T_LOCK && [rsi].tokval <= T_REPZ )
        ...
        .return asmerr(2068) .if [rsi].token != T_INSTRUCTION
    .endif
    ...
    .for ( j = 0: j < lengthof(opndx) && [rsi].token != T_FINAL: j++ )
        ...
        imul edi,j,expr
        lea rcx,opndx[rdi]
        .return .ifd EvalOperand( &i, tokenarray, Token_Count, rcx, 0 ) == ERROR
    ...
    .return asmerr(2008, [rsi].tokpos) .if [rsi].token != T_FINAL
    .if j >= 3
    ...
    .for ( q = 0, ebx = 0: ebx < j && ebx < MAX_OPND: ebx++, q++ )
        ...
        .if ( eax >= VEX_START && ebx == OPND2 && \
            ...
            lea r8,vex_flags
            movzx edi,byte ptr [r8+rax-VEX_START]
            ...
            .else
                ...
                .elseif ( ( edi & VX_DST ) && ( opndx[expr*2].kind == EXPR_CONST ) )
                    ...
                    .if opndx[OPND1].base_reg
                        ...
                        mov CodeInfo.rm_byte,0
                        .return .ifd process_register( &CodeInfo, OPND1, &opndx, q ) == ERROR
                        inc q
                    .endif
                .else
                    mov rcx,opndx[rdx].base_reg
        ...
        .case EXPR_ADDR
            .return .ifd process_address( &CodeInfo, ebx, &opndx[rdx] ) == ERROR
            .endc
        .case EXPR_CONST
            .return .ifd process_const( &CodeInfo, ebx, &opndx[rdx] ) == ERROR
            .endc
        .case EXPR_REG
        ...
        .if ebx > 1
            ...
            .if ebx > 2
                ...
                .while 1
                    ...
                    .break .if [r8+rax].opnd_class.opnd_type_3rd != OP3_NONE
            ...
            .if ( CodeInfo.token == T_IMUL )
                ...
                .endif
            .endif
            .return .ifd check_size( &CodeInfo, &opndx ) == ERROR
        .endif
