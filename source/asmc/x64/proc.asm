   ...
   .new fastcall_id:int_t = GetFastcallId( edi )
    ...
    .for ( cntParam = 0 : [rbx].token != T_FINAL : cntParam++ )
        ...
        .if ( paracurr )
            ...
            .if ( ParamReverse )
                .for ( rcx = [rsi].paralist: rcx && [rcx].dsym.nextparam != rdi \
                    : rcx = [rcx].dsym.nextparam )
                .endf
            .else
...
ParseProc proc __ccall uses rsi rdi rbx p:ptr dsym,
  ...
  .new Ofssize:byte
        ...
        .if ( [rbx].token == T_RES_ID && [rbx].tokval == T_FRAME )
            ...
            .if ( [rbx].token == T_COLON )
                ...
                .elseif ( [rax].asym.state != SYM_UNDEFINED && \
                       [rax].asym.state != SYM_INTERNAL && \
                       [rax].asym.state != SYM_EXTERNAL )
        ...
        .if ( eax == 'sesu' && byte ptr [rcx+4] == 0 )
            ...
            .for ( cnt = 0 : : cnt++, rbx += asm_tok )
                .if ( [rbx].token != T_REG )
                    ...
                    lea r8,SpecialTable
                    .break .if ( [r8+rax].special_item.type != RWT_REG )
    ...
    mov rdi,p
    .if ( i >= Token_Count || \
       ( [rbx].token == T_STRING && [rbx].string_delim == '{' ) )
...
ProcDir proc __ccall uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
  ...
  local p:ptr proc_info
    ...
    .if ( Parse_Pass == PASS_1 )
        ...
        .if ( [rdi].asym.state == SYM_EXTERNAL && [rdi].asym.flag1 & S_ISPROC )
            sym_ext2int( rdi )
...
WriteSEHData proc __ccall private uses rsi rdi rbx p:ptr dsym
   ...
   .new buffer[128]:char_t
    ...
    .if ( [rsi].exc_handler )
        mov rcx,[rsi].exc_handler
        AddLineQueueX( "dd IMAGEREL %s", [rcx].asym.name )
    ...
    AddLineQueueX( "dd IMAGEREL %s, IMAGEREL %s+0%xh, IMAGEREL $xdatasym+0%xh",
                  [rdi].asym.name, [rdi].asym.name, [rdi].asym.total_size, xdataofs )
    AddLineQueueX("%s ENDS", segname )
...
EndpDir proc __ccall uses rbx i:int_t, tokenarray:ptr asm_tok
  ...
  local buffer[128]:char_t
    ...
    mov rbx,rdx
    .if ( ecx != 1 || [rbx+2*asm_tok].token != T_FINAL )
        imul ecx,ecx,asm_tok
...
win64_MoveRegParam proc __ccall private uses rsi rdi i:int_t, size:int_t, param:ptr dsym
...
win64_GetRegParams proc __ccall private uses rsi rdi rbx varargs:ptr int_t, size:ptr inr_t, param:ptr dsym
    ...
    .endif
    mov rdx,varargs
    .if ( edi && [rdi].asym.sflags & S_ISVARARG )
        mov dword ptr [rdx],1
        .return 4
    .endif
    mov dword ptr [rdx],0
...
win64_SaveRegParams proc __ccall private uses rsi rdi rbx info:ptr proc_info
   ...
   .new maxregs:int_t = 4
    ...
    mov rdi,[rsi].paralist
    mov ebx,win64_GetRegParams( &varargs, &size, rdi )
    .while ( ebx > maxregs )
        dec ebx
        .if ( Parse_Pass == PASS_1 && !( [rdi].asym.sflags & S_ISVARARG ) && size >= 16 )
            mov eax,size
            mul ebx
            add eax,16
    ...
    .fors ( --ebx: rdi && ebx >= 0: ebx-- )
        ...
        .elseif ( ebx < 4 )  ; v2.09: else branch added
            ...
            add eax,8
            lea rcx,ms64_regs
            AddLineQueueX( "mov [%r+%u], %r", T_RSP, eax, [rcx+rbx*4] )
        .endif
    .endf
    ret
    ...
    .if ( [rsi].flags & PROC_ISFRAME )
        ...
        .if ( ModuleInfo.frame_auto & 1 )
            ...
            .else
                ...
                AddLineQueueX( " %r %r", T_DOT_PUSHREG, ebx )
                AddLineQueueX( " mov %r, %r", ebx, T_RSP )
                AddLineQueueX( " %r %r, 0", T_DOT_SETFRAME, ebx )
            .endif
...
if STACKPROBE
                .if ( Options.chkstack && ebx > 0x1000 )
                    ...
                    AddLineQueueX( [rdi+2*8], T_RAX, [rsi].localsize, [rcx].asym.name )
                    AddLineQueue( " externdef _chkstk:PROC" )
                    AddLineQueue( " call _chkstk" )
                    AddLineQueue( " sub rsp, rax" )
                .else
endif
                    AddLineQueueX( [rdi], T_RSP, [rsi].localsize, [rcx].asym.name )
if STACKPROBE
                .endif
endif
                mov rcx,sym_ReservedStack
                AddLineQueueX( [rdi+8], T_DOT_ALLOCSTACK, [rsi].localsize, [rcx].asym.name )
    ...
    .if ( ModuleInfo.Ofssize == USE64 && ( ModuleInfo.fctype == FCT_WIN64 || \
        ...
        mov rcx,sym_ReservedStack
        .if ( ModuleInfo.fctype == FCT_ELF64 && [rsi].flags & PROC_HAS_VARARG )
            mov [rcx].asym.value,208
        .endif
        mov resstack,[rcx].asym.value
    .endif
    ...
    .if ( sysstack || [rsi].locallist || \
        ...
        .if ( !( [rsi].flags & PROC_FPO ) )
            ...
            movzx ecx,ModuleInfo.Ofssize
            lea rdx,stackreg
            AddLineQueueX( " mov %r, %r", ebx, [rdx+rcx*4] )
        .endif
    .endif
...
if STACKPROBE
        ...
        .if ( ModuleInfo.epilogueflags )
            AddLineQueueX( "lea %r,[%r-(%d + %s)]", ebx, ebx, [rsi].localsize, [rcx].asym.name )
        .else
            AddLineQueueX( "sub %r, %d + %s", ebx, [rsi].localsize, [rcx].asym.name )
        .endif
if STACKPROBE
        .endif
endif
...
if STACKPROBE
        .if ( Options.chkstack && [rsi].localsize > 0x1000 )
            AddLineQueue( "externdef _chkstk:PROC" )
            AddLineQueueX( "mov eax, %d", [rsi].localsize )
            AddLineQueue( "call _chkstk" )
            .if ( ModuleInfo.Ofssize == USE64 )
        ...
        .elseif ( Options.masm_compat_gencode || [rsi].localsize == 128 )
...
if STACKPROBE
    ...
    .if ( [rsi].flags & PROC_LOADDS )
        ...
        AddLineQueueX( "mov ax, %s", &szDgroup )
        mov eax,T_AX
...
runqueue:
    ...
    .if ( ModuleInfo.Ofssize == USE64 && ModuleInfo.fctype == FCT_ELF64 && \
        ...
        AddLineQueueX( " mov DWORD PTR [rsp+8], %d", &[rbx*8] )
        AddLineQueue ( " mov DWORD PTR [rsp+12], 48" )
        .for ( : ebx < 6: ebx++ )
            lea rcx,elf64_regs
        ...
        .endf
        AddLineQueue( ".endif" )
        AddLineQueue( " lea rax, [rsp+224]" )
        AddLineQueue( " mov [rsp+16], rax" )
        AddLineQueue( " lea rax, [rsp+32]" )
        AddLineQueue( " mov [rsp+24], rax" )
        AddLineQueue( " lea rax, [rsp+8]" )
    .endif
...
SetLocalOffsets proc __ccall uses rsi rdi rbx info:ptr proc_info
   ...
   .new wsize:int_t = CurrWordSize
    ...
    mov rsi,info
    .if ( [rsi].flags & PROC_ISFRAME || ( ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) && \
   ...
   .new sreg:int_t
    ...
    .if ( [rsi].flags & PROC_ISFRAME )
        ...
        .if ( ModuleInfo.frame_auto & 1 )
            ...
            .if ( ( leav && [rsi].flags & PROC_PE_TYPE ) && ( cstack || \
                ...
                .if ( ( ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_VEC64 ) && \
                    ...
                    .if ( ModuleInfo.epilogueflags )
                        AddLineQueueX( "lea %r, [%r + %d + %s]", ebx,
                            ebx, [rsi].localsize, [rdx].asym.name )
                    .else
                        AddLineQueueX( "add %r, %d + %s", ebx, [rsi].localsize, [rdx].asym.name )
                    .endif
                .else
                    .if ( ModuleInfo.epilogueflags )
                        AddLineQueueX( "lea %r, [%r+%d]", ebx, ebx, [rsi].localsize )
                    .elseif ( [rsi].localsize )
    ...
    .if ( ModuleInfo.Ofssize == USE64 && ( ModuleInfo.fctype == FCT_WIN64 || \
        ...
        .elseif ( resstack || [rsi].localsize )
            ...
            mov rdx,sym_ReservedStack
            .if ( ModuleInfo.epilogueflags )
                AddLineQueueX( "lea %r, [%r + %d + %s]", ebx, ebx, [rsi].localsize, [rdx].asym.name )
            .else
                AddLineQueueX( "add %r, %d + %s", ebx, [rsi].localsize, [rdx].asym.name )
            .endif
        .endif
    ...
    .else
        ...
        .if ( [rsi].flags & PROC_FPO )
            ...
            .elseif ( [rsi].localsize )
                .if ( ModuleInfo.epilogueflags )
                    AddLineQueueX( "lea %r, [%r+%d]", sreg, sreg, [rsi].localsize )
                .else
