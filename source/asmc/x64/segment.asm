...
SetSymSegOfs proc uses rdi sym:ptr asym
    ...
    mov [rdi].asym.offs,GetCurrOffset()
    ret
...
FindClass proc private uses rsi rdi name:string_t, len:int_t
    ...
    .for ( rdi = ModuleInfo.LnameQueue.head: rdi: rdi = [rdi].qnode.next )
        ...
        .if ( [rsi].asym.state == SYM_CLASS_LNAME )
            .if ( SymCmpFunc( [rsi].asym.name, name, len ) == 0 )
                .return( rsi )
            .endif
        .endif
    .endf
...
EndsDir proc uses rbx i:int_t, tokenarray:ptr asm_tok
    ...
    mov rbx,rdx
    .if ( CurrStruct != NULL )
        .return( EndstructDirective( ecx, rdx ) )
    .endif
...
UnlinkSeg proc private dir:ptr dsym
    ...
    .for ( rdx = SymTables[TAB_SEG*symbol_queue].head, r8 = NULL : rdx : r8 = rdx, rdx = [rdx].dsym.next )
        .if ( rdx == rcx )
            ...
            .else
                mov [r8].dsym.next,[rdx].dsym.next
            .endif

            ; if segment is last, set a new tail

            .if ( [rdx].dsym.next == NULL )
                mov SymTables[TAB_SEG*symbol_queue].tail,r8
            .endif
        .endif
    .endf
    ret
