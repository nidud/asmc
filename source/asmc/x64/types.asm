...
EndstructDirective proc uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
    ...
    .if ( i == 1 )
        ...
        mov r8,[rdi].asym.name
        .if ( byte ptr [r8] == 0 )
            xor r8d,r8d
        .endif
        CreateStructField( -1, NULL, r8, MT_TYPE, rdi, [rdi].asym.total_size )
...
UpdateStructSize proc sym:ptr asym
    ...
    .elseif ( [rcx].asym.typekind == TYPE_UNION )
        .if ( [rdx].asym.total_size > [rcx].asym.total_size )
            mov [rcx].asym.total_size,[rdx].asym.total_size
        .endif
    .else
        add [rcx].asym.offs,[rdx].asym.total_size
        .ifs ( [rcx].asym.offs > [rcx].asym.total_size )
            mov [rcx].asym.total_size,[rcx].asym.offs
        .endif
    .endif
    ret
                ...
                .if ( [rdi].asym.typekind == TYPE_TYPEDEF )
                    ...
                    .if ( [rdi].asym.is_ptr == 0 )
                        ...
                        .if ( distance == FALSE && [rsi].is_ptr == 1 && \
                            ( [rdi].asym.mem_type == MT_NEAR || \
                              [rdi].asym.mem_type == MT_PROC || \
                              [rdi].asym.mem_type == MT_FAR ) )
    ...
    .if ( [rdi].asym.mem_type != MT_EMPTY )
        ...
        .if ( ti.mem_type != [rdi].asym.mem_type || \
            ...
            ( ti.is_far != [rdi].asym.is_far || bl || \
              ti.ptr_memtype != [rdi].asym.ptr_memtype || \
              rcx != rdx ) ) )
...
RecordDirective proc uses rsi rdi rbx i:int_t, tokenarray:ptr asm_tok
   ...
   .new offs:uint_t
    ...
    .elseif ( [rsi].asym.state == SYM_TYPE && \
            ( [rsi].asym.typekind == TYPE_RECORD || \
              [rsi].asym.typekind == TYPE_NONE ) )
    ...
    .repeat
        ...
        .if ( oldr )
            .if ( rsi == NULL || \
                [rsi].asym.state != SYM_STRUCT_FIELD || \
                [rsi].asym.mem_type != MT_BITS || \
