                   ...
                   .new namlen:int_t = eax
                    ...
                    .for ( edx = SymTables[TAB_SEG*symbol_queue].head : edx : edx = [edx].dsym.next )
                        ...
                        .if ( [edx].asym.name_size == namlen )
                            ...
                            push edx
                            mov ecx,sg
                            tmemcmp( [edx].asym.name, [ecx].asym.name, namlen )
                            pop edx
...
pe_create_MZ_header proc uses esi modinfo:ptr module_info
    ...
    .if ( Parse_Pass == PASS_1 )
        ...
        .for ( edi = SymTables[TAB_SEG*symbol_queue].head : edi : edi = [edi].dsym.next )
            ...
            .elseif ( [esi].segtype == SEGTYPE_UNDEF )
                ...
                .if ( tmemcmp( ebx, ".rsrc", 5 ) == 0 )
                    .if ( B[ebx+5] == 0 || B[ebx+5] == '$' )
                        mov [esi].segtype,SEGTYPE_RSRC
                    .endif
                .elseif ( tstrcmp( ebx, ".reloc" ) == 0 )
...
compare_exp proc p1:ptr expitem, p2:ptr expitem
    mov ecx,p1
    mov edx,p2
    .return( tstrcmp( [ecx].expitem.name, [edx].expitem.name ) )
compare_exp endp
...
pe_emit_export_data proc uses esi edi ebx
  ...
  local pitems:ptr expitem
    ...
    ;
    ; create .edata segment, export directory: Characteristics, Timedate, MajMin, Name, Base, ...
    ;
    AddLineQueueX(
        "option dotname\n"
        "%s segment dword %s\n"
...
ifndef ASMC64
    ...
    .for ( ebx = ModuleInfo.DllQueue: ebx: ebx = [ebx].next )
        ...
        .if ( [ebx].cnt )
            ...
            .endw
            ;
            ; import directory entry, emit ILT
            ;
            lea ecx,@CStr(idataname)
            lea edx,@CStr(idataattr)
            AddLineQueueX(
                ...
                "@%s_ilt label %r",
                ecx, edx, esi, esi, esi, ecx, ecx, cpalign, edx, esi, ptrtype )
            ...
            .endf
            ;
            ; ILT termination entry, emit IAT
            ;
            AddLineQueueX(
                "@LPPROC 0\n"
                "%s" IMPILTSUF " ends\n"
                "%s" IMPIATSUF " segment %s %s\n"
                "@%s_iat label %r", idataname, idataname, cpalign, idataattr, esi, ptrtype )
            ...
            .for ( edi = SymTables[TAB_EXT*symbol_queue].head : edi : edi = [edi].dsym.next )
                ...
                .if ( [edi].asym.flags & S_IAT_USED && [edi].asym.dll == ebx )
                    AddLineQueueX(
                        "@%s_name dw 0\n"
                        "db '%s',0\n"
                        "even", [edi].asym.name, [edi].asym.name )
                .endif
            .endf
            ...
            AddLineQueueX(
                "@%s_name db '%s',0\n"
                "even\n"
                "%s" IMPSTRSUF " ends", esi, &[ebx].name, idataname )
