...
FindToken proc private uses esi edi token:string_t, table:ptr string_t, size:int_t
...
SetModel proc private uses esi edi ebx
    ...
    .if ( ModuleInfo._model == MODEL_FLAT )
        lea esi,@CStr("FLAT")
    .endif
    AddPredefinedText( "@data", esi )

    .if ( ModuleInfo.distance == STACK_FAR )
        lea esi,@CStr("STACK")
    .endif
    AddPredefinedText( "@stack", esi )
...
ModelDirective proc uses esi edi ebx i:int_t, tokenarray:ptr asm_tok
  ...
  local init:byte
    ...
    imul ebx,i,asm_tok
    add ebx,tokenarray
    .return( asmerr( 2013 ) ) .if ( [ebx].token == T_FINAL )

    ; get the model argument

    xor esi,esi ; model

    FindToken( [ebx].string_ptr, &ModelToken, lengthof( ModelToken ) )
    .ifs ( eax >= 0 )
        lea esi,[eax+1] ;; model is one-base ( 0 is MODEL_NONE )
        add ebx,16
    ...
    .while ( [ebx].token == T_COMMA )
        ...
        .if ( [ebx].token != T_COMMA )
            ...
            mov i,0
            .if ( GetLangType( &i, ebx, &language ) == NOT_ERROR )
                add ebx,16
                mov cl,INIT_LANG
            .else
                FindToken( [ebx].string_ptr, &ModelAttr, lengthof( ModelToken ) )
                .break .ifs ( eax < 0 )

                mov cl,ModelAttrValue[eax*2].init
                mov al,ModelAttrValue[eax*2].value
                .if cl == INIT_STACK
                    .if ( esi == MODEL_FLAT )
                        .return( asmerr( 2178 ) )
                    .endif
...
CpuDirective proc uses ebx i:int_t, tokenarray:ptr asm_tok

    imul ebx,i,asm_tok
    add ebx,tokenarray

    mov edx,GetSflagsSp( [ebx].tokval )
    add ebx,16
    .if ( [ebx].token != T_FINAL )
        .return( asmerr(2008, [ebx].tokpos ) )
    .endif
...
SetCPU proc newcpu:cpu_info
    ...
    .if ( Options.output_format == OFORMAT_ELF )
        lea eax,elf64_fmtopt
    .else
        lea eax,coff64_fmtopt
    .endif
