...
TypeFromClassName proc uses esi edi s:ptr dsym, clname:ptr asym
  ...
  local uname[MAX_ID_LEN+1]:char_t
    ...
    .switch( esi )
    .case 5
        .if ( tmemcmp( edi, "CONST", 6 ) == 0 )
            .return( SEGTYPE_DATA )
        .endif
        .if ( tmemcmp( edi, "DBTYP", 6 ) == 0 )
            .return( SEGTYPE_DATA )
        .endif
        .if ( tmemcmp( edi, "DBSYM", 6 ) == 0 )
            .return( SEGTYPE_DATA )
        .endif
    .case 4
        .if ( tmemcmp( &[edi+esi-4], "CODE", 4 ) == 0 )
            .return( SEGTYPE_CODE )
        .endif
        .if ( tmemcmp( &[edi+esi-4], "DATA", 4 ) == 0 )
            .return( SEGTYPE_DATA )
        .endif
    .case 3
        .if ( tmemcmp( &[edi+esi-3], "BSS", 3 ) == 0 )
            .return( SEGTYPE_BSS )
        .endif
        .endc
    .default
        ...
        .case INIT_ALIAS
            ...
            .if ( is_old )
                ...
                .if ( [edi].seg_info.aliasname )
                    ...
                    inc ecx
                    tmemcmp( [edi].seg_info.aliasname, ecx, [ebx].stringlen )
                .endif
...
SortSegments proc uses esi edi ebx type:int_t
    ...
    .while ( changed == TRUE )
        ...
        .for ( edi = SymTables[TAB_SEG*symbol_queue].head: edi && [edi].dsym.next : esi = edi, edi = [edi].dsym.next )
            ...
            .case 1
                mov ecx,[edi].dsym.next
                .ifs ( tstrcmp( [edi].asym.name, [ecx].asym.name ) > 0 )
                    mov swap,TRUE
                .endif
                .endc
            .case 2
