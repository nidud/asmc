		...
		.for ebx = wfblk, edi = ff_count, esi = ff_table : edi : edi--, esi += 4
			...
			.if flags & _COMP_CREATE	; Compare File creation time
				...
				.if osopen( addr [edx].S_FBLK.fb_name, 0, M_RDONLY, A_OPEN ) != -1
					...
					push eax
					_close( ecx )
					__FTToTime(addr [ebx].WIN32_FIND_DATA.ftCreationTime)
					pop ecx
					.continue .if eax != ecx
				.endif
				mov	edx,[esi]
			...
			.if flags & _COMP_ACCESS	; Compare Last access time
				...
				.if osopen( addr [edx].S_FBLK.fb_name, 0, M_RDONLY, A_OPEN ) != -1
					...
					push eax
					_close( ecx )
					__FTToTime(addr [ebx].WIN32_FIND_DATA.ftLastAccessTime)
					pop ecx
					.continue .if eax != ecx
				.endif
				mov edx,[esi]
			...
			.if flags & _COMP_NAME	; Compare File names
				...
				.continue .if _stricmp(addr [ebx].WIN32_FIND_DATA.cFileName, ecx)
			.endif
...
local	fsize:qword
	...
	.if filter_wblk(edi)
		strfcat(addr path, directory, addr [edi].WIN32_FIND_DATA.cFileName)
		mov eax,ff_count
		.if eax < MAXHIT
			__FTToTime(addr [edi].WIN32_FIND_DATA.ftLastWriteTime)
			mov ecx,eax
			.if fballoc(addr path, ecx, fsize, [edi].WIN32_FIND_DATA.dwFileAttributes)
				mov	ecx,ff_count
				inc	ff_count
				shl	ecx,2
				add	ecx,ff_table
