...
wssort PROC USES esi edi ebx wsub:PTR S_WSUB
	...
	.if	eax > 1
		...
		.while	1
			...
			.if	SDWORD PTR compare( esi, ebx ) > 0
			...
			.if	SDWORD PTR compare( esi, edi ) > 0
			...
			.if	SDWORD PTR compare( ebx, edi ) > 0
			...
			.while	1
				...
				add	p,4
				.if	p < edi
					.continue .if SDWORD PTR compare( p, ebx ) <= 0
				.endif
				...
				.while	1
					...
					sub	n,4
					.break .if n <= ebx
					.break .if SDWORD PTR compare( n, ebx ) <= 0
				.endw
