...
PUBLIC		woutput_flush
PUBLIC		woutput_proctab
...
S_OUTPUT	ENDS
	...
	OPTION	PROC:PRIVATE, SWITCH:PASCAL
...
OPST_type:
	xor  eax,eax
	.for ecx = sizeof(formchar) : ecx : ecx--, eax++

		.if dl == formchar[eax]

			call woutput_proctab[eax*4]
			jmp  OUTPUT
		.endif
	.endf
	jmp OUTPUT
...
OUTPUT_b:
	.for edx = output_getd(), ecx = 32 : ecx : ecx--
	...
	.for : ecx : ecx -= 2
		...
		shr edx,1
		adc al,'0'
		mov [ebp].OP_buffer[ecx-2],ax
	.endf
	jmp OUTPUT_LDTEXT

OUTPUT_c:
	call	output_getd
	mov	[ebp].OP_buffer,ax
	mov	[ebp].OP_count,1
...
@06:
	...
	.if esi & FLAG_FORCEOCTAL
		...
		.if WORD PTR [edx] != '0' || [ebp].OP_count == 0
...
OUTPUT_MULTI PROC USES esi edi
	...
	movzx eax,ax
	.fors esi = eax, edi = edx : edi > 0 : edi--
...
OUTPUT_STRING PROC USES esi edi
	...
	.fors esi = ecx, edi = edx : esi > 0 : esi--, edi += 2
...
OUTPUT	PROC
	...
	.if eax == [ebp].OP_no_output
		...
		.ifs ecx > 0
			...
			.fors edx = [ebp].OP_text : ecx > 0 : ecx--, edx+=2
...
_woutput PROC USES edx ecx esi edi ebx filep:LPFILE, format:LPWSTR, args:PVOID
