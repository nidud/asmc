; MKTIME.ASM--
;
; Copyright (c) The Asmc Contributors. All rights reserved.
; Consult your license regarding permissions and restrictions.
;

include time.inc

    .data
    .code

ChkAdd macro dest, src1, src2
    exitm<( ((src1 !>= 0) && (src2 !>= 0) && (dest !< 0)) || ((src1 !< 0) && (src2 !< 0) && (dest !>= 0)) )>
    endm

ChkMul macro dest, src1, src2
    .if src1
        mov eax,dest
        xor edx,edx
        div src1
        cmp eax,src2
        mov eax,0
        setne al
    .else
        xor eax,eax
    .endif
    retm<eax>
    endm

_make_time_t proc private uses esi edi ebx tb:ptr tm, ultflag:int_t

  local tmptm1:long_t, tmptm2:long_t, tmptm3:long_t
  local tbtemp:ptr tm

    mov edi,tb
    mov eax,[edi].tm.tm_year
    .if ( ( eax < _BASE_YEAR - 1) || ( eax > _MAX_YEAR + 1) )
        jmp err_mktime
    .endif
    mov tmptm1,eax

    mov eax,[edi].tm.tm_mon
    mov ecx,12
    xor edx,edx
    div ecx
    mov tmptm2,eax

    .if ( eax != 0 )
        add tmptm1,eax
        mov [edi].tm.tm_mon,edx
        .ifs ( edx < 0 )
            add [edi].tm.tm_mon,12
            dec tmptm1
        .endif
        .if ( (tmptm1 < _BASE_YEAR - 1) || (tmptm1 > _MAX_YEAR + 1) )
            jmp err_mktime
        .endif
    .endif

    mov eax,[edi].tm.tm_mon
    mov ecx,_days[eax]
    .if ( !(tmptm1 & 3) && (eax > 1) )
        inc ecx
    .endif
    mov tmptm2,ecx

    mov eax,tmptm1
    mov ecx,eax
    sub eax,_BASE_YEAR
    imul eax,eax,365
    dec ecx
    shr ecx,2
    add eax,ecx
    sub eax,_LEAP_YEAR_ADJUST
    add eax,tmptm2
    mov tmptm3,eax

    mov ecx,[edi].tm.tm_mday
    mov tmptm2,ecx
    lea edx,[ecx+eax]
    mov tmptm1,edx
    .ifs ( ChkAdd(edx, eax, ecx) )
        jmp err_mktime
    .endif

    imul eax,tmptm1,24
    mov tmptm2,eax
    .if ( ChkMul(tmptm2, tmptm1, 24) )
        jmp err_mktime
    .endif

    mov eax,[edi].tm.tm_hour
    mov tmptm3,eax
    add eax,tmptm2
    mov tmptm1,eax
    .if ( ChkAdd(tmptm1, tmptm2, tmptm3) )
        jmp err_mktime
    .endif

    imul eax,tmptm1,60
    mov tmptm2,eax
    .if ( ChkMul(tmptm2, tmptm1, 60) )
        jmp err_mktime
    .endif

    mov eax,[edi].tm.tm_min
    mov tmptm3,eax
    add eax,tmptm2
    mov tmptm1,eax
    .if ( ChkAdd(tmptm1, tmptm2, tmptm3) )
        jmp err_mktime
    .endif

    imul eax,tmptm1,60
    mov tmptm2,eax
    .if ( ChkMul(tmptm2, tmptm1, 60) )
        jmp err_mktime
    .endif

    mov eax,[edi].tm.tm_sec
    mov tmptm3,eax
    add eax,tmptm2
    mov tmptm1,eax
    .if ( ChkAdd(tmptm1, tmptm2, tmptm3) )
        jmp err_mktime
    .endif

    .if  ( ultflag )

        _tzset()
        add tmptm1,_timezone

        mov esi,localtime(&tmptm1)
        .if ( eax == NULL )
            jmp err_mktime
        .endif

        .if ( ([edi].tm.tm_isdst > 0) || (([edi].tm.tm_isdst < 0) && ([esi].tm.tm_isdst > 0)) )
            sub tmptm1,3600
            mov esi,localtime(&tmptm1)
        .endif

    .else
        .if ( gmtime(&tmptm1) == NULL )
            jmp err_mktime
        .endif
        mov esi,eax
    .endif

    mov ecx,sizeof(tm)
    rep movsb

    .return tmptm1
err_mktime:
    .return (-1)

_make_time_t endp

mktime proc tb:ptr tm
    .return( _make_time_t(tb, 1) )
mktime endp

_mkgmtime proc tb:ptr tm
    .return( _make_time_t(tb, 0) )
_mkgmtime endp

    end
