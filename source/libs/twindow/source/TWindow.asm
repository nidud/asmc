...
TWindow::Open proc uses rsi rbx rc:TRect, flags:uint_t

    mov rsi,GetProcessHeap()
   .return .if HeapAlloc(rsi, HEAP_ZERO_MEMORY, TWindow) == NULL

    mov rbx,rax
    mov rcx,this

    mov [rbx].lpVtbl,[rcx].lpVtbl
    mov [rbx].Class,[rcx].Class
    mov [rbx].Color,[rcx].Color
    mov [rbx].rc,rc
    mov [rbx].Flags,flags
    mov [rbx].Prev,rcx
    .if [rbx].Flags & W_TRANSPARENT
        mov [rbx].Color,&AttributesTransparent
    .endif
    .return rbx .if [rbx].Flags & W_CHILD

    movzx eax,[rbx].rc.col
    movzx edx,[rbx].rc.row
    mov ecx,eax
    mul dl
    lea r8,[rax*4]
...
TWindow::Load proc uses rsi rdi rbx p:ptr TResource

    mov     rbx,rcx
    lea     rsi,[rdx].TResource.dialog
    movzx   eax,[rsi].TRObject.count
    lea     rsi,[rsi+rax*8+8]
    mov     r10,[rbx].Color
    mov     rdi,[rbx].Window
    add     rdi,2
...
TWindow::Child proc rc:TRect, id:uint_t, type:uint_t

    .if this.Open(edx, W_CHILD or W_WNDPROC)

        mov rcx,rax
        mov [rcx].Index,id
        mov [rcx].Type,type
        mov [rcx].WndProc,&TWindow_DefWindowProc
        mov rax,rcx
        mov rcx,[rcx].Prev
        .while [rcx].Child
            mov rcx,[rcx].Child
...
TWindow::Resource proc uses rsi rdi rbx p:ptr TResource
  ...
  local i:int_t
    ...
    .for ( i = 1 : edi : edi--, rsi += TRObject, i++ )

        mov     r9w,[rsi].flags
        and     r9d,0x0F
        inc     r9d
        mov     rcx,[rbx].Child([rsi].rc, i, r9d)
        movzx   eax,[rsi].flags
        and     eax,0xFFF0
        shl     eax,4
        or      [rcx].Flags,eax
...
TWindow::SetShade proc uses rsi rdi

    .if ( [rcx].Flags & W_SHADE )

       .new r1:TRect, r2:TRect
...
TWindow::ClrShade proc uses rdi
...
TWReadLine proc private uses rsi rdi rbx this:ptr TWindow, x:int_t, y:int_t, l:int_t

  local rc:SMALL_RECT, size:int_t
    ...
    .if HeapAlloc(GetProcessHeap(), 0, size)
...
TWWriteLine proc private uses rbx this:ptr TWindow, x:int_t, y:int_t, l:int_t, p:PCHAR_INFO
...
TWindow::Move proc uses rsi rdi rbx r12 x:int_t, y:int_t
  ...
  local lx          :int_t,
    ...
    .return .if !HeapAlloc(GetProcessHeap(), 0, rsi)

    .for wp = rax, Direction = 0 : count : count--
...
TWindow::Read proc rc:ptr TRect, pc:PCHAR_INFO
...
TWindow::Write proc rc:ptr TRect, pc:PCHAR_INFO
...
TWindow::Exchange proc rc:ptr TRect, pc:PCHAR_INFO
...
TWindow::MoveConsole proc uses rcx x:int_t, y:int_t
    ...
    SetWindowPos( GetConsoleWindow(), 0, x, y, 0, 0,
...
TWindow::MoveConsole endp
    ...
    assume rsi:ptr TClass

TWindow::SetConsole proc uses rsi rdi rbx rcx cols:int_t, rows:int_t
...
TWindow::CPutBackground endp
    ...
    assume rdx:ptr TMessage

Dispatch proc private uses rcx hwnd:ptr TMessage, msg:ptr TMessage

    mov r9,[rcx].TWindow.Class
    mov r8,[rdx].Next
    mov rax,[r9].TClass.Message

    .if ( rax == rdx )

        mov [r9].TClass.Message,r8
...
Translate proc private uses rdi rcx hwnd:ptr TWindow, msg:ptr TMessage
...
TWindow::Send endp
    ...
    assume r10:ptr TClass

TWindow::Post proc uiMsg:uint_t, wParam:size_t, lParam:ptr

    .return .if !HeapAlloc(GetProcessHeap(), 0, TMessage)
...
TWindow::Register proc uses rsi rdi rbx tproc:TWINPROC

  local Count:dword
  local Event:INPUT_RECORD
  local hPrev:ptr TWindow
...
GetItemRect proto :ptr TWindow {
    mov     rax,[this].Prev
    movzx   eax,word ptr [rax].TWindow.rc
    add     eax,[this].rc
    }

ContextRect proto :ptr TWindow {
    GetItemRect(this)
    mov [this].Context.rc,eax
    }


OnEnterIdle proc uses rcx hwnd:ptr TWindow
...
Inside proc hwnd:ptr TWindow, pos:COORD
...
Inside endp
    ...
    assume rsi:ptr TContext

OnLButtonDown proc uses rsi rdi rbx rcx hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
...
OnLButtonUp proc uses rsi rdi rbx rcx hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
...
OnMouseMove proc uses rsi hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
...
OnSetFocus proc uses rsi rdi rbx rcx hwnd:ptr TWindow
...
OnKillFocus proc uses rsi rdi rbx rcx hwnd:ptr TWindow
...
NextItem proc uses rcx hwnd:ptr TWindow
...
PrevItem proc uses rbx rcx hwnd:ptr TWindow
...
ItemRight proc uses rcx hwnd:ptr TWindow
...
ItemLeft proc uses rcx hwnd:ptr TWindow
...
OnChar proc uses rbx rcx hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
...
OnSysChar proc uses rcx hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
...
WndProc proc hwnd:ptr TWindow, uiMsg:uint_t, wParam:size_t, lParam:ptr
    ...
    .switch edx
      ...
      .case WM_CLOSE
        .return [rcx].Release()
    .endsw
    .return [rcx].DefWindowProc(edx, r8, r9)
...
TWindow::InitInstance proc uses rsi rdi rbx
  ...
  local ci: CONSOLE_SCREEN_BUFFER_INFO
    ...
    .if eax
        ...
        mov [rdi].Focus,    1
        mov [rdi].Console,  [rsi].Console
        mov [rdi].StdIn,    [rsi].StdIn
        mov [rdi].StdOut,   [rsi].StdOut
        mov [rdi].ErrMode,  [rsi].ErrMode
        mov [rdi].ConMode,  [rsi].ConMode
    ...
    .if HeapAlloc(rsi, 0, TWindowVtbl) == NULL
    ...
    mov [rbx].Color,&AttributesDefault
    mov rdi,[rbx].Class
    ...
    GetConsoleScreenBufferInfo([rdi].StdOut, &ci)

    assume rdi:nothing

    cmp     eax,0
    mov     eax,0x00190050
    cmovnz  eax,ci.dwSize
    ...
    mov     [rbx].rc.row,al
    mul     ecx
    mov     [rbx].Window,HeapAlloc(rsi, 0, &[rax*4])
...
TWindow::TWindow proc uses rsi rdi rbx
    ...
    .if HeapAlloc(rsi, HEAP_ZERO_MEMORY, TWindow + TClass) == NULL
    ...
    .if rdi
        stosq
    .endif
    mov [rbx].Class,&[rbx + TWindow]

    .if TWindow::InitInstance(rbx)
...
TWindow::Release proc uses rsi rbx
    ...
    .if m_cRef == 0
        ...
        HeapFree(rsi, 0, [rbx].lpVtbl)
        mov m_base,NULL
    .endif

    .if [rbx].Cursor

        HeapFree(rsi, 0, [rbx].Cursor)
    .endif
