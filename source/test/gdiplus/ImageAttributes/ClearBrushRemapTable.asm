...
CreateMetafile proc hdc:HDC
  ...
  local FullTranslucent:ARGB
   ...
   .new m:Metafile(L"TestMetafile4.emf", hdc)
   .new g:Graphics(&m)
   .new p:GraphicsPath()
   .new a:Pen(Red, 4.0)

    mov count,1
    mov FullTranslucent,Red

    g.SetSmoothingMode(SmoothingModeAntiAlias)
    p.AddEllipse(20, 40, 150, 70)
    g.DrawPath(&a, &p)
    p.Release()
    a.Release()
   ...
   .new p:GraphicsPath()
    ...
    p.AddEllipse(200, 40, 150, 70)

   .new b:PathGradientBrush(&p)

    b.SetCenterColor(Red)
    b.SetSurroundColors(&FullTranslucent, &count)
    g.FillEllipse(&b, 200, 40, 150, 70)
...
WndProc proc uses rsi rdi rbx hWnd:HWND, message:UINT, wParam:WPARAM, lParam:LPARAM
