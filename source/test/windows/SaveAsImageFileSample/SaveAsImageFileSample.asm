...
SaveToImageFile proc
   ...
   .new lg:D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES
    ...
    mov hr,CoCreateInstance(&CLSID_WICImagingFactory, NULL,
            CLSCTX_INPROC_SERVER, &IID_IWICImagingFactory, &pWICFactory)
    .if (SUCCEEDED(hr))
        mov hr,D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, &IID_ID2D1Factory, NULL, &pD2DFactory)
    .endif
    .if (SUCCEEDED(hr))
        mov hr,DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, &IID_IDWriteFactory, &pDWriteFactory)
    .endif
    ...
    .if (SUCCEEDED(hr))
        mov hr,pWICFactory.CreateBitmap(sc_bitmapWidth, sc_bitmapHeight, &GUID_WICPixelFormat32bppBGR,
                WICBitmapCacheOnLoad, &pWICBitmap)
    .endif

    .if (SUCCEEDED(hr))
        mov rcx,D2D1::RenderTargetProperties()
        mov hr,pD2DFactory.CreateWicBitmapRenderTarget(pWICBitmap, rcx, &pRT)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov hr,pDWriteFactory.CreateTextFormat(sc_fontName, NULL, DWRITE_FONT_WEIGHT_NORMAL,
            DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, sc_fontSize, L"", &pTextFormat)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov hr,pRT.CreateGradientStopCollection(&stops, 2, D2D1_GAMMA_2_2,
    ...
    .if (SUCCEEDED(hr))
        ...
        mov rcx,D2D1::BrushProperties()
        mov hr,pRT.CreateLinearGradientBrush(&lg, rcx, pGradientStops, &pLGBrush)
    .endif

    .if (SUCCEEDED(hr))
        mov hr,pRT.CreateSolidColorBrush(Color.Init(Black, 1.0), NULL, &pBlackBrush)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        pRT.DrawText(sc_helloWorld, 13, pTextFormat, &rc, pBlackBrush,
