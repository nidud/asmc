...
DemoApp::Initialize proc uses rdi hInstance:HINSTANCE
    ...
    .if (SUCCEEDED(hr))
        ...
        mov hr,CoCreateInstance(&CLSID_WICImagingFactory, NULL, CLSCTX_INPROC_SERVER, &IID_IWICImagingFactory, &m_pIWICFactory)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov m_hWnd,CreateWindowEx(0, "WICANIMATEDGIF",  "WIC Animated Gif Sample", WS_OVERLAPPEDWINDOW or WS_VISIBLE,
                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, rdi, rbx)
        mov hr,E_FAIL
        .if rax
            mov hr,S_OK
        .endif
   ...
   .new size:D2D1_SIZE_U
    ...
    .if (SUCCEEDED(hr))
        ...
        .if ( m_pHwndRT == NULL )
           ...
           .new hwndRenderTragetproperties:D2D1_HWND_RENDER_TARGET_PROPERTIES(m_hWnd, edx, ecx)
            mov hr,m_pD2DFactory.CreateHwndRenderTarget(&renderTargetProperties, &hwndRenderTragetproperties, &m_pHwndRT)
    ...
    .if (SUCCEEDED(hr))
        ...
        SafeRelease(m_pFrameComposeRT)
        cvtsi2ss xmm0,m_cxGifImage
        cvtsi2ss xmm1,m_cyGifImage
        movss size.width,xmm0
        movss size.height,xmm1
        mov hr,m_pHwndRT.CreateCompatibleRenderTarget(&size, NULL, NULL,
...
DemoApp::OnRender proc
   ...
   .new pRT:ptr ID2D1HwndRenderTarget = m_pHwndRT

    ;; Check to see if the render targets are initialized

    .if ( rax && m_pFrameComposeRT )
        ...
        .if (SUCCEEDED(hr))
            ...
            .if !( eax & D2D1_WINDOW_STATE_OCCLUDED )
               ...
               .new drawRect:D2D1_RECT_F
                mov hr,CalculateDrawRectangle(&drawRect)

                .if (SUCCEEDED(hr))

                    ;; Get the bitmap to draw on the hwnd render target

                    mov hr,m_pFrameComposeRT.GetBitmap(&pFrameToRender)
                .endif
...
s_WndProc proc hWnd:HWND, uMsg:UINT, wParam:WPARAM, lParam:LPARAM
    ...
    .if ( ldr(uMsg) == WM_NCCREATE )
       ...
       .return( 1 )
    .endif
    .new app:ptr DemoApp = GetWindowLongPtr(ldr(hWnd), GWLP_USERDATA)
    .if rax
...
DemoApp::GetGlobalMetadata proc
   ...
   .new hr:HRESULT
    ...
    mov hr,m_pDecoder.GetFrameCount(&m_cFrames)
    .if (SUCCEEDED(hr))

        ;; Create a MetadataQueryReader from the decoder

        mov hr,m_pDecoder.GetMetadataQueryReader(&pMetadataQueryReader)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        GetBackgroundColor(pMetadataQueryReader)
        .if (FAILED(eax))

            ;; Default to transparent if failed to get the color

            lea rcx,m_backgroundColor
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .if (SUCCEEDED(hr))
                ...
                mov m_cxGifImage,propValue.uiVal
            .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .endif
            .if (SUCCEEDED(hr))
                mov m_cyGifImage,propValue.uiVal
            .endif
            PropVariantClear(&propValue)
        .endif
    .endif
            ...
            .if (SUCCEEDED(hr))
                ...
                .if ( eax != 0 )
                    ...
                    .if ( xmm0 > 1.0 )
                        ...
                        divss    xmm1,xmm0
                        cvtss2si eax,xmm1
                        mov m_cyGifImagePixel,eax
                        mov m_cxGifImagePixel,m_cxGifImage
                    .else

                        cvtsi2ss xmm1,m_cxGifImage
                        mulss    xmm0,xmm1
                        cvtss2si eax,xmm0
                        mov m_cxGifImagePixel,eax
                        mov m_cyGifImagePixel,m_cyGifImage
                    .endif
                .else
...
DemoApp::GetRawFrame proc uFrameIndex:UINT
   ...
   .new propValue:PROPVARIANT
    ...
    mov hr,m_pDecoder.GetFrame(uFrameIndex, &pWicFrame)
    ...
    .if (SUCCEEDED(hr))
        ...
        SafeRelease(m_pRawFrame)
        mov hr,m_pHwndRT.CreateBitmapFromWicBitmap(pConverter, NULL, &m_pRawFrame)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .if (SUCCEEDED(hr))
                ...
                cvtsi2ss xmm0,eax
                movss m_framePosition.left,xmm0
            .endif
            PropVariantClear(&propValue);
        .endif
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .if (SUCCEEDED(hr))
                ...
                movzx eax,propValue.uiVal
                cvtsi2ss xmm0,eax
                movss m_framePosition.top,xmm0
            .endif
            PropVariantClear(&propValue)
        .endif
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .if (SUCCEEDED(hr))
                ...
                movzx eax,propValue.uiVal
                cvtsi2ss xmm0,eax
                addss xmm0,m_framePosition.left
                movss m_framePosition.right,xmm0
            .endif
            PropVariantClear(&propValue)
        .endif
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(hr))
            ...
            .if (SUCCEEDED(hr))
                ...
                movzx eax,propValue.uiVal
                cvtsi2ss xmm0,eax
                addss xmm0,m_framePosition.top
                movss m_framePosition.bottom,xmm0
            .endif
            PropVariantClear(&propValue)
        .endif
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(eax))
            ...
            .if (SUCCEEDED(hr))
                ...
                mov hr,UIntMult(propValue.uiVal, 10, &m_uFrameDelay)
            .endif
            PropVariantClear(&propValue)
    ...
    .if (SUCCEEDED(hr))
        ...
        .if (SUCCEEDED(eax))
            ...
            .endif
            .if (SUCCEEDED(hr))
                mov m_uFrameDisposal,propValue.bVal
            .endif
        ...
        .else
            ...
            mov m_uFrameDisposal,DM_UNDEFINED
        .endif
    .endif
    PropVariantClear(&propValue)
...
DemoApp::GetBackgroundColor proc pMetadataQueryReader:ptr IWICMetadataQueryReader
   ...
   .new pWicPalette:ptr IWICPalette = NULL
    ...
    .if (SUCCEEDED(hr))
        mov hr,m_pIWICFactory.CreatePalette(&pWicPalette)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov hr,m_pDecoder.CopyPalette(pWicPalette)
    .endif
...
;
    ...
    assume rsi:ptr D2D1_RECT_F

DemoApp::CalculateDrawRectangle proc uses rsi drawRect:ptr D2D1_RECT_F
   ...
   .new rcClient:RECT
    ...
    .if (SUCCEEDED(hr))
        ...
        cvtsi2ss xmm1,m_cxGifImagePixel
        cvtsi2ss xmm2,m_cyGifImagePixel
        ...
        movss [rsi].top,xmm0
        addss xmm1,[rsi].left
        movss [rsi].right,xmm1
        addss xmm0,xmm2
        movss [rsi].bottom,xmm0
...
DemoApp::RestoreSavedFrame proc
   ...
   .new pFrameToCopyTo:ptr ID2D1Bitmap = NULL
    ...
    .if ( m_pSavedFrame )
        mov hr,m_pFrameComposeRT.GetBitmap(&pFrameToCopyTo)
    .endif
    .if (SUCCEEDED(hr))

        ;; Copy the whole bitmap

        mov hr,pFrameToCopyTo.CopyFromBitmap(NULL, m_pSavedFrame, NULL)
    .endif
...
DemoApp::ClearCurrentFrameArea proc

   .new pRT:ptr ID2D1BitmapRenderTarget = m_pFrameComposeRT

    pRT.BeginDraw()

    ;; Clip the render target to the size of the raw frame

    pRT.PushAxisAlignedClip(&m_framePosition, D2D1_ANTIALIAS_MODE_PER_PRIMITIVE)
    pRT.Clear(&m_backgroundColor)
...
DemoApp::OverlayNextFrame proc

    ;; Get Frame information

    .new pRT:ptr ID2D1BitmapRenderTarget = m_pFrameComposeRT
    .new hr:HRESULT = GetRawFrame(m_uNextFrameIndex)
    ...
    .if (SUCCEEDED(hr))
        ...
        pRT.DrawBitmap(m_pRawFrame, &m_framePosition, 1.0, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, NULL)
        mov hr,pRT.EndDraw(NULL, NULL)
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov eax,m_uNextFrameIndex
        cdq
        div m_cFrames
        mov m_uNextFrameIndex,edx
    .endif
    .return( hr )
    endp
...
DemoApp::SaveComposedFrame proc

    .new pFrameToBeSaved:ptr ID2D1Bitmap = NULL
    .new hr:HRESULT = m_pFrameComposeRT.GetBitmap(&pFrameToBeSaved)
    ...
    .if (SUCCEEDED(hr))
        ...
        .if ( m_pSavedFrame == NULL )
           ...
           .new bitmapProp:D2D1_BITMAP_PROPERTIES
            ...
            mov hr,m_pFrameComposeRT.CreateBitmap(bitmapSize, NULL, 0, &bitmapProp, &m_pSavedFrame)
        .endif
    .endif
    ...
    .if (SUCCEEDED(hr))
        ...
        mov hr,m_pSavedFrame.CopyFromBitmap(NULL, pFrameToBeSaved, NULL)
    .endif
...
DemoApp::SelectAndDisplayGif proc
...
ifndef _UNICODE
    ...
    .if GetFileOpen(rcx, ARRAYSIZE(szFileName))

        ;; Reset the states

        mov m_uNextFrameIndex,0
        mov m_uFrameDisposal,DM_NONE ;; No previous frame, use disposal none
        mov m_uLoopNumber,0
        mov m_fHasLoop,FALSE
...
ifndef _UNICODE
        ...
        mov hr,m_pIWICFactory.CreateDecoderFromFilename(rdx, NULL, GENERIC_READ, WICDecodeMetadataCacheOnLoad, &m_pDecoder)

        .if (SUCCEEDED(hr))
            mov hr,GetGlobalMetadata()
        .endif

        .if (SUCCEEDED(hr))

            mov rcClient.right,m_cxGifImagePixel
            mov rcClient.bottom,m_cyGifImagePixel
        ...
        .if (SUCCEEDED(hr))
            ...
            MoveWindow(m_hWnd, rcWindow.left, rcWindow.top, edx, ecx, TRUE)
...
DemoApp::ComposeNextFrame proc
   ...
   .new hr:HRESULT = S_OK
    ...
    .if ( m_pHwndRT && m_pFrameComposeRT )
        ...
        mov hr,DisposeCurrentFrame()
        .if (SUCCEEDED(hr))

            mov hr,OverlayNextFrame()
        .endif
...
DemoApp::RecoverDeviceResources proc
    ...
    mov m_uNextFrameIndex,0
    mov m_uFrameDisposal,DM_NONE ;; No previous frames. Use disposal none.
    mov m_uLoopNumber,0
