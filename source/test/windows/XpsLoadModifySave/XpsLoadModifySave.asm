...
LoadAttachedResourceToMemory proc streamResourceId:LPCTSTR, lpType:LPCTSTR, ppData:ptr, pdwSize:PDWORD

   .new dwSize:DWORD = 0
   .new hData:HGLOBAL = NULL
   .new pData:ptr = NULL
   .new hTestModule:HMODULE = NULL
   .new hr:HRESULT = S_OK
   .new hRes:HRSRC
...
GetReadStreamFromAttachedResource proc lpName:LPCTSTR, lpType:LPCTSTR, ppReadStream:ptr ptr IStream, pdwSize:PDWORD

   .new pBuf:ptr = NULL
   .new pData:ptr = NULL
   .new pReadStream:ptr IStream = NULL
   .new hr:HRESULT
...
GetXpsFactory proc factory:ptr ptr IXpsOMObjectFactory
    .return CoCreateInstance(&CLSID_XpsOMObjectFactory, NULL, CLSCTX_INPROC_SERVER, &IID_IXpsOMObjectFactory, factory)
    endp


LoadXpsPackage proc xpsStreamResourceId:WORD, pXpsFactory:ptr IXpsOMObjectFactory, ppXpsPackage:ptr ptr IXpsOMPackage
  local hr:HRESULT, pStream:ptr IStream, size:DWORD
    mov pStream,NULL
    mov hr,GetReadStreamFromAttachedResource(rcx, RC_PACKAGE_DATA, &pStream, &size)
    .if ( SUCCEEDED(hr) )
        mov hr,pXpsFactory.CreatePackageFromStream(pStream, FALSE, ppXpsPackage)
    .endif
    .if ( pStream )
        pStream.Release()
        mov pStream,NULL
...
FindFirstPageReference proc pXpsFactory:ptr IXpsOMObjectFactory, pXpsPackage:ptr IXpsOMPackage,
...
CreateWaterMark proc pXpsFactory:ptr IXpsOMObjectFactory, pXpsFontResource:ptr IXpsOMFontResource, ppXpsCanvas:ptr ptr IXpsOMCanvas

  local hr:HRESULT, pXpsGlyphs:ptr IXpsOMGlyphs,
...
AddWatermarkToPage proc pXpsPage:ptr IXpsOMPage, pXpsCanvas:ptr IXpsOMCanvas
  local pVisualCollection:ptr IXpsOMVisualCollection, hr:HRESULT
...
GetFontResource proc pXpsPageReference:ptr IXpsOMPageReference, ppXpsFontResource:ptr ptr IXpsOMFontResource

  local hr:HRESULT, pXpsPartResources:ptr IXpsOMPartResources, pXpsFontResCollection:ptr IXpsOMFontResourceCollection
...
SerializePackage proc pXpsPackage:ptr IXpsOMPackage
  local szDesktopPath[MAX_PATH]:WCHAR, hr:HRESULT
...
wmain proc argc:int_t, argv:ptr ptr wchar_t
  ...
  local hr                : HRESULT,
    ...
    .else
        mov hr,LoadXpsPackage(IDR_RC_PACKAGE_DATA1, pXpsFactory, &pXpsPackage)
    .endif
    .if ( !SUCCEEDED(hr) )
