...
alloc_string PROC USES edi value:sdword
  local base
	invoke	alloca,MAXTARGET
	mov	edi,eax
	invoke	ExpandEnvironmentStrings,value, edi, MAXTARGET-1
	invoke	salloc,edi
	ret
alloc_string ENDP
...
addsymbol PROC USES edi symbol:ptr sbyte, value:sdword
	invoke	findsymbol, symbol
	test	eax,eax
	jz	@F
	mov	edi,edx
	invoke	free,eax
	invoke	alloc_string,value
	test	eax,eax
	jz	error
	jmp	set_value
@@:
...
@@:
	...
	mov	[ecx-1],al
	mov	[edx],al
	invoke	findsymbol,symbol_name
	test	eax,eax
	jnz	@F
	mov	eax,offset null_string
@@:
	invoke	strxchg,esi, symbol_macro, eax
	dec	edi
	jmp	loop_$
error:
	invoke	perror,string
	invoke	exit,1
toend:
	invoke	salloc,esi
	ret
...
case_if_loop:	; !if <> == <> || <> ..
	...
	.switch al
	  ...
	  .case '>'
		...
		jmp	case_if_true
	  .case '&'
		invoke	findsymbol,edi
		test	eax,eax
		jz	case_if_false
		cmp	BYTE PTR [eax],'0'
		je	case_if_false
		invoke	findsymbol,esi
		test	eax,eax
		jz	case_if_false
		cmp	BYTE PTR [eax],'0'
		je	case_if_false
...
build_target PROC USES esi edi ebx target:ptr S_TARGET
  local base,
	...
	mov	edi,eax
	mov	esi,target
	invoke	expandsymbol,[esi].S_TARGET.t_string
	mov	esi,eax
	mov	target_string,eax
@@:
	lodsb
...
@@:
	mov	BYTE PTR [edi],0
	mov	edi,base
	invoke	findtarget,edi
	test	eax,eax
	jnz	do_target
	invoke	build_object,edi
	jmp	do_objects

do_target:
	add	esp,MAXTARGET
	invoke	build_target,eax
	sub	esp,MAXTARGET
	test	eax,eax
	jz	do_objects
	jmp	toend
