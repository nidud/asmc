...
info_1	db "sub rsp",0
info_2	db "push rbp/leave",0
info_3	db "push rbp/pop rbp",0
info_4	db "4.asm",0
info_5	db "5.asm",0
info_6	db "6.asm",0
info_7	db "7.asm",0
...
GetCycleCount proc uses rsi rdi rbx l1, l2, step, count
	...
	lea rdi,total
	.while 1
		.for ebx = 0, ecx = 0, edx = -1 : ecx < 10 : ecx++
			mov rax,[rsi+rcx*8]
			.if eax && eax < edx
				mov rdx,rax
				mov rbx,rcx
	...
	.while 1
		.for r8d = 0, ecx = 0, edx = -1 : ecx < 10 : ecx++
			mov eax,[rdi+rcx*8]
			.if eax && eax < edx
				mov rdx,rax
				mov r8,rcx
...
mainCRTStartup proc
	...
	.endf
	lea rdx,cpustring
	.for : BYTE PTR [rdx] == ' ' : rdx++
	.endf
	mov eax,sselevel
	mov r8,@CStr( "SSE2" )
	.switch
	  .case eax & SSE_AVX2:	 mov r8,@CStr( "AVX2" )
	  .case eax & SSE_AVX:	 mov r8,@CStr( "AVX" )
	  .case eax & SSE_SSE42: mov r8,@CStr( "SSE4.2" )
	  .case eax & SSE_SSE41: mov r8,@CStr( "SSE4.1" )
	  .case eax & SSE_SSSE3: mov r8,@CStr( "SSSE3" )
	  .case eax & SSE_SSE3:	 mov r8,@CStr( "SSE3" )
	.endsw
	printf( "%s (%s)\n----------------------------------------------\n", rdx, r8 )

	.if !VirtualProtect( addr proc_x, size_p, PAGE_EXECUTE_READWRITE, addr lpflOldProtect )
